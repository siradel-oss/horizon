...
* A grid can now be displayed when translating a gizmo along a plane. It can be configured using the `grid` field of [GizmoLayer]($proto).
* Each [representation](reference/HrzProtocol.VectorRepr) now has a mandatory `id` field that must be unique to the layer. This ID is now used with the `emit_id` instruction instead of the index of the representation.
* The background color of the planet can now be changed using the `terrain_color` field of [TerrainSettings]($proto), which is part of [SceneViewSettings]($proto).

# Changed

* **Rasters**
    * The property `layersIds` on [ArcGisRasterProviderParams]($proto) has been renamed to `layerIds`.
...
* **Vectors**
    * The `name` of each [VectorRepr]($proto) is now optional but can still be used with the `emit` or `emit_named` instructions.
...
* **3D Tiles**
    * The `data_texture_palette` field of [ThreeDTilesLayer]($proto) has been removed. Its value must be specified in a [Material]($proto) in the `materials` field.
...
* **Rasters**
    * Set the layer ID list on [ArcGisRasterProviderParams]($proto) structures with the `layerIds` property instead of `layersIds`.
...
* The `id` field of each [VectorRepr]($proto) must be filled with a unique ID per layer.
* The `emit` instruction in styling scripts should be replaced by `emit_named`. When possible, it is even better to use `emit_id`.
* The `terrain_color` field of [TerrainSettings]($proto) in each [SceneViewSettings]($proto) should be set to `r = 0, g = 0, b = 1, a = 1` to match the previous color.
