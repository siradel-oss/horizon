...
* Added the `GetSceneViewScaleAndAltitude` method to [CameraService]($proto) to retrieve information about a scene view.
    * The [returned information](reference/HrzProtocol.ViewScaleAltitude) is the scale on the view (in pixels per meter), and the relative and absolute altitude of the camera.
    * This can be used to implement a map scale on the viewer.
* Added the `draw_wireframe_from_position` property to [ViewshedSettings]($proto), to draw viewshed wireframes from the viewpoint position, instead of the near plane.
...
* **Rasters**
    ...
    * The [NodataValue]($proto) structure now has a `type` property of type [NodataValueType]($proto), as well as dedicated properties for values of each type.
    ...
    * The `geometry` field is now a member of the relevant raster providers parameters ([TiledRasterProviderParams]($proto) and [UntiledRasterProviderParams]($proto)) instead of being a member of `RasterParams`. For all other providers the geometry parameters are determined from the source data and the values of [RasterGeometry]($proto) were already ignored.
    * The `tiling_scheme` field has been removed from `RasterGeometry` and added to the only provider that uses it: [TiledRasterProviderParams]($proto).
    * The `sampling`, `blending`, and `display_bounds` fields of `RasterParams` have been moved to [Raster]($proto).
    * The `RasterParams` structure has been removed.
    * The nodata value is now specified in the `nodata` field of each raster provider parameter structure, instead of being part of [RasterSampling]($proto). The value is now inside a [RasterNodata]($proto) structure, that allows explicitly enabling or disabling the nodata value.
    * All provider parameter fields (`*_provider`) have been moved from [Raster]($proto) and [PalettizedRasterProviderParams]($proto) to a new structure named [RasterProvider]($proto). The new field is named `provider` in both [Raster]($proto) and [PalettizedRasterProviderParams]($proto).
    * The field `type` of [RasterProvider]($proto) allows explicitly setting the type of the provider.
    * The field `nodata_color` has been added on [PalettizedRasterProviderParams]($proto). It allows setting the colour for nodata values coming from the child provider, regardless of what the actual values are.
    * The field `nodata` has been added on [PalettizedRasterProviderParams]($proto). It allows specifying a colour (after palettisation) that is considered nodata when the layer is drawn on the globe. This colour can be either displayed or discarded according to the value of the `nodata_handling` field of [RasterSampling]($proto).
    * All raster providers have been renamed:
        * `TiledImageRasterProviderParams` to [TiledRasterProviderParams]($proto).
        * `SingleImageRasterProviderParams` to [UntiledRasterProviderParams]($proto).
        * `PalettizedImageProviderParams` to [PalettizedRasterProviderParams]($proto).
        * `BingProviderParams` to [BingRasterProviderParams]($proto).
        * `ArcGisProviderParams` to [ArcGisRasterProviderParams]($proto).
        * `TmsProviderParams` to [TmsRasterProviderParams]($proto).
        * `WmtsProviderParams` to [WmtsRasterProviderParams]($proto).
        * `WmsProviderParams` to [WmsRasterProviderParams]($proto).
        * `CesiumTerrainProviderParams` to [CesiumTerrainRasterProviderParams]($proto).
        * `TileJsonProviderParams` to [TileJsonRasterProviderParams]($proto).
        * `PmTilesProviderParams` to [PmTilesRasterProviderParams]($proto).
    * All values of [RasterProviderType]($proto) have been renamed:
    ...
    * Some fields of [RasterProvider]($proto) have been renamed:
        * `tiled_image` to `tiled`.
        * `single_image` to `untiled`.
        * `palettized_image` to `palettized`.
* **3D models**
    * The [`SIRADEL_data_texture`](SIRADEL_data_texture.html) glTF extension has been extended to detail how data bits from colour images are reinterpreted as scalar values. (What [ImageFormat]($proto) is used for in raster layers.) Entries of the `textures` array in glTF JSONs for data textures must now have a `SIRADEL_data_texture` extension object with a value for the `dataInterpretation` property. The only accepted value is `rgba8BitsToFloat`, which corresponds to the `R_F32` image format.
...
* **Picking**
    * The fields `imagery`, `dtm`, and `scalar` of [RasterPickResult]($proto) have been replaced with `color` and `number`. Depending on the type of each raster that has been picked, the corresponding field gets filled. The image format of the raster is given explicitly in the `image_format` field.
        * `ImageryPickResult`, `DtmPickResult`, and `ScalarPickResult` have been removed.
    * The `type` field of [RasterPickResult]($proto) has been removed, as well as the `RasterType` enumeration. Instead colour and scalar imagery rasters can be distinguished with the `image_format` field and the type of the result. Imagery rasters and DTM rasters can be distinguished with the contents of the value in the `layer` field of [PickLayerResult]($proto).
...
* **Rasters**
    ...
    * Any nodata value that was set on a palettised raster should now be set on its scalar value child provider. If relying on the `nodata_handling` value of [RasterSampling]($proto) is desired, use the `nodata_color` field of [PalettizedImageRasterProviderParams]($proto) to choose how to palettise nodata values.
* **3D models**
