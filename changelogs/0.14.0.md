...
* Added the value of data textures to the picking result of single models and 3D Tiles. See the `data_texture_value` field of [SingleModelPickingResult]($proto) and [ThreeDTilesPickResult]($proto).
* The width of flat overlay polylines can now be expressed in meters or pixel using the `line_width_unit` field of [FlatOverlayVectorRepr]($proto).
...
* **Events**
    * Key bindings for actions such as re-orienting the camera, toggling the development UI or deselecting all the features can now be specified in [ViewerOptions]($proto). See [KeyAction]($proto) for the full list of key-bindable actions.
    * Added `ToggleDevUi` and `MoveDevUiToCursor` methods to [ViewerService]($proto) to open and move the development UI programmatically.
    * Added a `DeleteSelectedControlPoint` method to [ShapeEditorService]($proto).
    * Added a `ResetNorth` method to [CameraService]($proto) to reorient the camera towards the north pole in accordance with the manipulator used.
* **Rasters**
...
* Added a new [SceneDumpService]($proto) with two methods (`DumpScene` and `LoadDump`). This service can be used to dump the entire scene model and state of the engine, and restore it later. This is mainly used for debugging and sharing scenes between developers. Do not use this as a user-facing format.

# Changed

* **Events**
    * Actions triggered by key presses, such as 'R' to reorient the camera or 'P' to toggle the development UI, are no longer set by default but need to be specified in the [ViewerOptions]($proto).
    * Web: Keyboard events that are not used by the engine are not captured anymore.
* **Scene dumps TypeScript library**
    * This library is now just a façade in front of the [SceneDumpService]($proto). It can be used to dump and load scene dumps in three formats: JSON, binary, or base64.
...
* Keys that trigger actions, such as 'P' for the development UI, must now be explicitly passed in [ViewerOptions]($proto).
