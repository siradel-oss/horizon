...
* **Rasters**
    * Added support for [PMTiles rasters](/doc/raster_providers.html#pmtiles).
    * Added the ability to use a [TileJson raster](/doc/raster_providers.html#tilejson) in the [palettized provider](palettized_image_raster.html).
* **Vector data**
    * Added [AttributeTransform]($proto)s to apply transformations to attributes upon reading them.
...
* **Styling**
    ...
    * Added the `to_string`, `to_int`, `to_uint`, `to_number`, `to_boolean`, and `to_color` functions, that act like [AttributeTransform]($proto), but in styling scripts.
* **Heatmaps**
    * Added a new `accumulation` parameter to [HeatmapVectorRepr]($proto) to specify how the value of points are accumulated.
...
* Added two new [camera notifications](reference/HrzProtocol.CameraNotification): `motion_started` and `motion_ended`, to detect when a camera starts and stops moving for any reason: animation, automated movement, user movement, etc.
...
* **Vector data**
    ...
    * Colors must be encoded as `0xaabbggrr` in numbers, or as `#rrggbbaa` or HTML color names in strings with the `TO_COLOR` [AttributeTransform]($proto).
...
* **Vector data**
    * In [AttributeValue]($proto) and [InMemoryAttributeValue]($proto):
        ...
        * `color_value` is now `number_value` with `0xaabbggrr`, or `string_value` with the `TO_COLOR` [AttributeTransform]($proto).
