...
* **Client-provided vector data**
    * Client providers of [[VectorDataSource]] can now be used to request tile geometries from the client, and is no longer restricted to attribute values.
        * This is controlled by the `has_geometry` field of [[VectorDataSource]].
        * A [[VectorDataRequestMessage]] message is sent through the message queue when a vector tile is required. They must be answered using the `ProvideVectorData` method of [[ClientDataService]].
        * Tile geometries can be invalidated to be requested again with the [[InvalidateVectorData]] method.
        * Geometry cannot be requested if the `access` field of [[ClientVectorDataProviderParams]] is not set to `ACCESS_BY_TILE`.
...
* Removed the `color` styling property of [[ExtrudedGeometryVectorRepr]].
