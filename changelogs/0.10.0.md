...
* **Camera**
    * The `camera_settings` field should be removed from [[SceneSettings]] and set into the new [[CameraSettings]] scene model root.
    * The `GetCameraViewBox` and `GetCameraViewPolygon` methods of the [[CameraService]] should use `SCENE_VIEW_0` in their [[SceneViewReference]] parameter.
    * The `LatLonAltToPixelCoords` method of the [[CameraService]] should use `SCENE_VIEW_0` in its [[GeographicPositionInSceneView]] parameter.
    * Bounds in the [[BoundsVisibilityConstraint]] structure must be converted to [[Wgs84Bounds]]. `x_min`, `y_min`, `x_max`, and `y_max` respectively become `west`, `south`, `east`, and `north`.
* **Multiview**
...
* **Clipping**
    * All new `clip_id` properties in [[VectorTilesLayer]], [[ThreeDTilesLayer]], [[SingleModelLayer]] and [[TerrainSettings]] must be set to -1.
* **Vectors**
    * Bounds in the [[VectorDataLayer]] and [[VectorTilesSource]] structures must be converted to [[Wgs84Bounds]]. See the note about [[BoundsVisibilityConstraint]] above.
* **Aesthetics**
