...
* **Camera**
    * Added a `continuous_movement_interruption` field to [[CameraMovement]] to specify [how to handle interruptions](HrzProtocol.CameraContinuousMovementInterruption.html) by other events.
...
* Editable shape line segment can now follow either [geodesics](https://en.wikipedia.org/wiki/Great-circle_distance) (which they always did before) or [rhumb-lines](https://en.wikipedia.org/wiki/Rhumb_line), which are lines of constant bearing. Two types of rhumb lines are available: those that cross the antimeridian, and those that cannot. This is configured through the `line_type` property of [[EditableShapeGeometry]].
