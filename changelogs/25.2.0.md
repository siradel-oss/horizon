...
* Added a new parameter `bevel_width` to [ExtrudedGeometryVectorRepr]($proto) which adds rounded corners to extruded polygons and polylines.
...
* The background colour of the loading screen can be configured with the property `loading_screen_background_color` of [ViewerOptions]($proto).
...
* **Colours**
    ...
    * Values `PERCEPTUAL_OKLAB` and `LINEAR_RGB` of [ColorInterpolationMode]($proto) have been renamed `OKLAB` and `LINEAR_SRGB` respectively. (Bringing the names closer to their CSS equivalents.)
    * Value `SRGB_R_8` of [ImageFormat]($proto) has been renamed `R8`.
* **Symbols**
...
* **Colours**
    ...
    * Uses of the values `PERCEPTUAL_OKLAB` and `LINEAR_RGB` of [ColorInterpolationMode]($proto) must be replaced with `OKLAB` and `LINEAR_SRGB` respectively.
    * Uses of the value `SRGB_R_8` of [ImageFormat]($proto) must be replaced with `R8`.
