...
* **Symbols**
    * Added the `COVER` mode to [BoxFit]($proto).
    * Added a [BoxFit]($proto) parameter to decorated shapes & image symbol elements.
...
* **Symbols**
    * The `ImageFit` enum and its values has been renamed to [BoxFitAxes]($proto).
    * The `fit` parameter of [ImageSymbolElement]($proto) has been renamed to `fit_axes`.
...
* The `fit` parameter of [ImageSymbolElement]($proto) must be renamed to `fit_axes`.
* To reproduce the previous layouting behaviour, the `fit_mode` parameters of [ImageSymbolElement]($proto) and [DecoratedShapeSymbolElement]($proto) should be set to `BOX_FIT_FILL`.
* A duration before a client vector data request timeouts and fails has been introduced in version 23.3.0, with a fixed duration of 5 seconds. This duration is now configurable through the `timeout` field of [ClientVectorDataProviderParams]($proto). The value `0` (which is the default) means no timeout: the client can take as much time as it needs before responding to a request. Set the value to `5` to reproduce the behaviour of version 23.3.0.
