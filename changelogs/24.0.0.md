...
* **Vector data**
    * Added the new [`UNTILED_VECTOR_DATA_PROVIDER`](reference/HrzProtocol.UntiledVectorDataProviderParams).
        ...
        * The amount of tiles generated is based on the [VectorDataLayer]($proto)’s `min_level`, `max_level` and `bounds`.
    ...
    * Added the `clip_to_tile` option to [ExtrudedGeometryVectorRepr]($proto).
    * Added scene views settings at the root of [VectorTilesLayer]($proto).
    ...
    * Flat overlays (flat vector representations and heatmaps) can now be drawn over 3D Tiles layers, with the property `draw_under_flat_overlays` of [ThreeDTilesLayer]($proto).
    * Flat overlays can be disabled by setting the property `disable_flat_overlays` of [ViewerOptions]($proto) to `true`. This can be used to lower video memory usage when the client does not need the flat overlay functionality.
    * Added support for dash period, length, and speed in pixels for cylinders.
* **Symbols**
    * Added screen-space symbols culling.
        * Three new flags have been added to [AnchorSymbolElement]($proto) to control this behaviour: `can_overlap_other_symbols`, `hides_other_symbols`, and `is_optional`.
    * Added a new `keep_upright` option to [AnchorSymbolElement]($proto) to keep symbols readable when they would normally look flipped or upside down.
    * Added `position_offset_relative_scaling` and `element_size_relative_scaling` to [AnchorSymbolElement]($proto), which only applies if the size unit of the corresponding property is set to `SYMBOL_SIZE_IN_PIXELS`.
...
* **Mapbox scenes**
    * Added the new [MapboxService]($proto), that can be used for translating Mapbox scenes into Horizon scenes. Support for Mapbox layers is still limited and works on a best-effort basis.
...
* The property `disable_terrain` of [ViewerOptions]($proto) no longer disables flat overlays when enabled. It still makes them invisible over the terrain, as there is no terrain, but they are visible when drawn over 3D Tiles.
...
* Removed [AnchorSymbolElement]($proto)’s `position_offset_is_relative` and `element_size_is_relative`. See `position_offset_relative_scaling` and `element_size_relative_scaling` instead.
...
* The `scene_views` field of [VectorTilesLayer]($proto) should be set to show all views to preserve the behaviour of previous versions.
* [AnchorSymbolElement]($proto)’s `position_offset_is_relative` and `element_size_is_relative` booleans have been replaced by the `position_offset_relative_scaling` and `element_size_relative_scaling` properties.
