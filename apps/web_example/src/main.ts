...
async function addOrthoLayer(api: HrzApi.AsyncApi) {
    let layerModel = HrzProtocol.ImageryRasterLayer.create({
        raster: {
            provider: {
                wmts: {
...
async function addDtmLayer(api: HrzApi.AsyncApi) {
    let layerModel = HrzProtocol.DtmRasterLayer.create({
        raster: {
            provider: {
                tiled: {
                    urlPattern:
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Deliberately left without an id: exercises HrzCoreBackend's fallback that
    // auto-assigns one when the canvas doesn't have one.
    ...
    HrzCoreBackend.init(
        ...
        "wasm/",
        options,
        async function (backend: HrzCoreBackend | null, initStatus: HrzProtocol.ViewerInitStatus) {
            if (initStatus !== HrzProtocol.ViewerInitStatus.INIT_SUCCESS || !backend) {
