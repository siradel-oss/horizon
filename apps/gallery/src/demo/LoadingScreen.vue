...
async function onHorizonReady(api: HrzApi.AsyncApi, msgHandler_: MessageHandler) {
    ...
    msgHandler.watchForever((msg) => {
        if (msg.payload == "viewerLoadingProgress") {
            const completedStepCount = msg.viewerLoadingProgress.completedStepCount || 0;
            const totalStepCount = msg.viewerLoadingProgress.totalStepCount || 0;
            loadingProgress.value = totalStepCount > 0 ? completedStepCount / totalStepCount : 0;
        } else if (msg.payload == "viewerReady") {
                    ...
                    <div class="relative flex items-center justify-center">
                        <svg width="100" height="100" class="relative z-10">
                            <circle
                                ...
                                r="40"
                                fill="none"
                                stroke="var(--theme-color-primary-container)"
                                stroke-opacity="0.3"
                                stroke-width="4"
                            />
                            <circle
                        ...
                        <div
                            class="absolute w-[56px] h-[56px] rounded-full bg-[var(--theme-color-primary-container)] animate-disc-pulse"
                        ></div>
                    </div>
                </div>
            </div>
