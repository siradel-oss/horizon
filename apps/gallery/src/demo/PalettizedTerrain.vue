...
const LEGEND_MIN = -10000;
const LEGEND_MAX = 8000;
const LEGEND_RANGE = LEGEND_MAX - LEGEND_MIN;

let api: HrzApi.AsyncApi;
let imageryLayer = shallowRef<HrzProtocol.LayerHandle | undefined>();
let dtmLayer = shallowRef<HrzProtocol.LayerHandle | undefined>();

let polyTopo = ref(true);
let polyBathy = ref(true);
let discrete = ref(false);
let includeDtmLayer = ref(false);

const legendColors = ref<HTMLElement | null>(null);
const legendValues = ref<HTMLElement | null>(null);
...
function valToPct(v: number): number {
    return ((v - LEGEND_MIN) / LEGEND_RANGE) * 100;
}

function buildLegend() {
    if (!legendColors.value || !legendValues.value) return;

    const stops: string[] = [];

    if (discrete.value) {
        if (polyBathy.value) {
            for (let i = 0; i < bathymetryColorStops.length - 1; i++) {
                const a = valToPct(bathymetryColorStops[i].value).toFixed(2);
                const b = valToPct(bathymetryColorStops[i + 1].value).toFixed(2);
                stops.push(`${colorToCss(bathymetryColorStops[i].color)} ${a}% ${b}%`);
            }
        } else {
            const cut = valToPct(bathymetryTopometryCutoffValue).toFixed(2);
            stops.push(`${colorToCss(bathymetryMonochromeColor)} 0% ${cut}%`);
        }
        const cut = valToPct(bathymetryTopometryCutoffValue).toFixed(2);
        if (polyTopo.value) {
            stops.push(`${colorToCss(topographyColorStops[0].color)} ${cut}%`);
            for (let i = 1; i < topographyColorStops.length; i++) {
                const a = valToPct(topographyColorStops[i - 1].value).toFixed(2);
                const b = valToPct(topographyColorStops[i].value).toFixed(2);
                stops.push(`${colorToCss(topographyColorStops[i - 1].color)} ${a}% ${b}%`);
            }
            const last = colorToCss(topographyColorStops[topographyColorStops.length - 1].color);
            stops.push(`${last} 100%`);
        } else {
            stops.push(`${colorToCss(topographyMonochromeColor)} ${cut}% 100%`);
        }
    } else {
        if (polyBathy.value) {
            for (const p of bathymetryColorStops) {
                stops.push(`${colorToCss(p.color)} ${valToPct(p.value).toFixed(2)}%`);
            }
        } else {
            const cut = valToPct(bathymetryTopometryCutoffValue).toFixed(2);
            const c = colorToCss(bathymetryMonochromeColor);
            stops.push(`${c} 0%`, `${c} ${cut}%`);
        }
        if (polyTopo.value) {
            for (const p of topographyColorStops) {
                stops.push(`${colorToCss(p.color)} ${valToPct(p.value).toFixed(2)}%`);
            }
        } else {
            const cut = valToPct(bathymetryTopometryCutoffValue).toFixed(2);
            const c = colorToCss(topographyMonochromeColor);
            stops.push(`${c} ${cut}%`, `${c} 100%`);
        }
    }

    legendColors.value.style.background = `linear-gradient(to top in oklab, ${stops.join(", ")})`;

    const LABEL_VALUES = [8000, 5000, 2000, 500, 0, -500, -2000, -5000, -10000];
    legendValues.value.innerHTML = "";
    for (const v of LABEL_VALUES) {
        const el = document.createElement("span");
        el.textContent = v.toLocaleString();
        el.style.cssText = `position:absolute; top:${(100 - valToPct(v)).toFixed(2)}%; right:0; transform:translateY(-50%);`;
        legendValues.value.appendChild(el);
    }
}

function currentPaletteEnum(): Palette {
    if (!polyTopo.value && !polyBathy.value) return Palette.MONO_MONO;
    if (polyTopo.value && polyBathy.value)
        return discrete.value ? Palette.POLY_POLY_DISCRETE : Palette.POLY_POLY_CONTINUOUS;
    if (polyTopo.value && !polyBathy.value)
        return discrete.value ? Palette.POLY_MONO_DISCRETE : Palette.POLY_MONO_CONTINUOUS;
    return discrete.value ? Palette.MONO_POLY_DISCRETE : Palette.MONO_POLY_CONTINUOUS;
}
...
function makeNumericPalette(palette: Palette): HrzProtocol.NumericPalette {
    let colorStops: HrzProtocol.ColorStop.$Properties[] = [];

    switch (palette) {
        case Palette.POLY_POLY_CONTINUOUS:
            for (let i = 0; i < bathymetryColorStops.length; i++) {
                colorStops.push({
                    value: bathymetryColorStops[i].value,
                    firstColor: bathymetryColorStops[i].color,
                    secondColor: bathymetryColorStops[i].color,
                });
            }
            colorStops[colorStops.length - 1].secondColor = topographyColorStops[0].color;
            for (let i = 1; i < topographyColorStops.length; i++) {
                colorStops.push({
                    value: topographyColorStops[i].value,
                    firstColor: topographyColorStops[i].color,
                    secondColor: topographyColorStops[i].color,
                });
            }
            break;
        case Palette.POLY_MONO_CONTINUOUS:
            for (let i = 0; i < topographyColorStops.length; i++) {
                colorStops.push({
                    value: topographyColorStops[i].value,
                    firstColor: topographyColorStops[i].color,
                    secondColor: topographyColorStops[i].color,
            ...
            colorStops[0].firstColor = bathymetryMonochromeColor;
            break;
        case Palette.MONO_POLY_CONTINUOUS:
            ...
            colorStops[colorStops.length - 1].secondColor = topographyMonochromeColor;
            break;
        case Palette.POLY_POLY_DISCRETE:
            for (let i = 0; i < bathymetryColorStops.length - 1; i++) {
                colorStops.push({
                    value: bathymetryColorStops[i].value,
                    firstColor: bathymetryColorStops[i].color,
                    secondColor: bathymetryColorStops[i + 1].color,
                });
            }
            colorStops.push({
                value: bathymetryTopometryCutoffValue,
                firstColor: bathymetryColorStops[bathymetryColorStops.length - 1].color,
                secondColor: topographyColorStops[0].color,
            });
            for (let i = 1; i < topographyColorStops.length; i++) {
                colorStops.push({
                    value: topographyColorStops[i].value,
                    firstColor: topographyColorStops[i - 1].color,
                    secondColor: topographyColorStops[i].color,
                });
            }
            break;
...
watch([polyTopo, polyBathy, discrete], async function () {
    ...
    await HrzApi.ImageryRasterLayerPathBuilder.create(imageryLayer.value)
        ...
        .palettized()
        .palette()
        .set(api, makeNumericPalette(currentPaletteEnum()));
    ...
    <SplitView>
        <template #left>
            <div class="typography-normal">
                ...
                <p>
                    <label for="topo-select">Topography</label>
                    <select id="topo-select" v-model="polyTopo">
                        <option :value="true">Polychromatic</option>
                        <option :value="false">Monochromatic</option>
                    </select>
                </p>
                <p>
                    <label for="bathy-select">Bathymetry</label>
                    <select id="bathy-select" v-model="polyBathy">
                        <option :value="true">Polychromatic</option>
                        <option :value="false">Monochromatic</option>
                    </select>
                </p>
                <p>
                    <label for="discrete-select">Color interpolation</label>
                    <select
                        id="discrete-select"
                        v-model="discrete"
                        :disabled="!polyTopo && !polyBathy"
                    >
                        <option :value="false">Continuous</option>
                        <option :value="true">Discrete</option>
                    </select>
                </p>
                <p>
                    <label>
        ...
        <template #right>
            ...
            <div
                class="absolute top-4 right-4 bottom-44 flex flex-col justify-center pointer-events-none"
            >
                <div
                    class="bg-surfaceContainerLow text-onSurface rounded-lg p-3 shadow-xl w-[110px]"
                >
                    <p class="text-mLabelMedium text-center mb-2">Elevation (m)</p>
                    <div class="relative flex gap-2 h-[350px]">
                        <div
                            class="relative flex-1 text-mLabelSmall text-right"
                            ref="legendValues"
                        ></div>
                        <div class="w-[28px] rounded-sm flex-shrink-0" ref="legendColors"></div>
                    </div>
                </div>
            </div>
        </template>
