...
let inMemoryLayer: HrzProtocol.LayerHandle;
let vectorDataLayer: HrzProtocol.LayerHandle;
let vectorTilesLayer: HrzProtocol.LayerHandle;
...
function markerToInMemoryFeature(marker: Marker): HrzProtocol.InMemoryVectorFeature {
...
async function schedulePick(x: number, y: number) {
    ...
    if (requestResult.hasATicket && requestResult.ticket) {
        msgHandler.awaitPickResult(
            ...
            (results: HrzProtocol.PickResults.$Shape) => {
                    ...
                    if (eqLong(result.layer?.handle?.opaque, vectorTilesLayer.opaque)) {
                        ...
                        if (idAttribute !== null && idAttribute !== undefined) {
                            selectedMarkerId.value =
                                HrzProtocolHelper.attributeAsNumber(idAttribute);
                        } else {
                            selectedMarkerId.value = null;
                        }
                        return;
