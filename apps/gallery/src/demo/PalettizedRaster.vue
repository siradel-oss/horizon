...
let layer = shallowRef<HrzProtocol.LayerHandle | undefined>();
...
let colors = reactive<HrzProtocol.Color.$Properties[]>([]);
...
watch(layer, async function (newLayer) {
    ...
    let colorStops = rasterData.provider?.palettized?.palette?.colorStops;
    if (colorStops) {
        colorStops.sort((a, b) => (a.value || 0) - (b.value || 0));

        for (let [index, value] of colorStops.entries()) {
...
watch([colors, paletteMode], async function () {
    ...
    if (paletteMode.value == PaletteMode.GRADIENT) {
        for (let [i, color] of colors.entries()) {
            paletteData.colorStops[i].value = colorValues[i];
            paletteData.colorStops[i].firstColor = i == 0 ? {} : color;
            paletteData.colorStops[i].secondColor = i == colors.length - 1 ? {} : color;
        }
    } else {
        for (let [i, _] of colors.entries()) {
            paletteData.colorStops[i].value = colorValues[i];
            paletteData.colorStops[i].firstColor = i == 0 ? {} : colors[i - 1];
            paletteData.colorStops[i].secondColor = i == colors.length - 1 ? {} : colors[i];
        }
    }
...
async function retrievePaletteModel(): Promise<any> {
    ...
    let paletteData = await HrzApi.ImageryRasterLayerPathBuilder.create(layer.value)
        ...
        .palette()
        .get(api);
    paletteData.colorStops?.sort((a, b) => (a.value || 0) - (b.value || 0));
...
async function schedulePick(x: number, y: number) {
    ...
    if (requestResult.hasATicket && requestResult.ticket) {
        msgHandler.awaitPickResult(
            requestResult.ticket,
            (pickResult: HrzProtocol.PickResults.$Shape) => {
...
async function scheduleRasterDataFetch(position: HrzProtocol.GeographicPosition.$Properties) {
    if (!layer.value) {
        return;
    }

    let rasterRequest = HrzProtocol.RasterDataFetchRequest.create({
        position: position,
    ...
    if (fetchRequest.hasATicket && fetchRequest.ticket) {
        msgHandler.awaitRasterDataFetchResult(
            fetchRequest.ticket,
            (result: HrzProtocol.PickLayerResult.$Shape[]) => {
        ...
        <template #left>
            <div class="typography-normal">
                ...
                <p>
                    ...
                    <a href="../doc/numeric_palettes.html"
