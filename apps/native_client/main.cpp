...
#undef OVERRIDE_SETTING_WITH_OPT_ARG
    ...
    {
        // @Safety Taking .data() of the string_view returned by _Name() is safe here because
        // it is backed by a static null-terminated string.
        printf(
    ...
    if (!mapbox_style_filename.empty())
    {
        ...
        if (fp)
        {
            ...
            params.set_style_json(std::string_view{raw_style.data(), raw_style.size()});
