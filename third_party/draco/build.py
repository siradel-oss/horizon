...
def ignore_all_but_headers(dir: str, names: list[str]) -> list[str]:
    return [
        name
        for name in names
        if not name.endswith(".h") and not (Path(dir) / name).is_dir()
    ]
...
for build_type in ["Release", "Debug"]:
    ...
    copy_file(
        build_dir.path / "draco/draco_features.h",
        install_dir / "include/draco/draco_features.h",
    )

    os.makedirs(install_dir / f"lib/{build_type}", exist_ok=True)
    if target_platform == "windows":
        copy_file(
            build_dir.path / f"{build_type}/draco.lib",
            install_dir / f"lib/{build_type}/draco.lib",
        )
    else:
        copy_file(
            build_dir.path / "libdraco.a", install_dir / f"lib/{build_type}/libdraco.a"
        )
