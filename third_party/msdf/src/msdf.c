...
{
    if (fabs(a) < 1e-14) return solve_quadratic(x, b, c, d);
...
int pixel_clash(const vec3 a, const vec3 b, double threshold)
{
    ...
    if ((a[0] > .5f) != (b[0] > .5f) && (a[0] < .5f) != (b[0] < .5f))
    {
        ...
        (a[1] > .5f) != (b[1] > .5f) && (a[1] < .5f) != (b[1] < .5f) && (a[2] > .5f) != (b[2] > .5f)
        && (a[2] < .5f) != (b[2] < .5f))
    {
        aa = a[1], ba = b[1];
        ab = a[2], bb = b[2];
...
signed_distance_t linear_dist(edge_segment_t* e, vec2 origin, double* param)
{
    ...
    *param = vec2_mul_inner(aq, ab) / vec2_mul_inner(ab, ab);
    ...
    if (*param > 0 && *param < 1)
    {
        ...
        if (fabs(ortho_dist) < endpoint_distance) return (signed_distance_t){ortho_dist, 0};
    }
...
signed_distance_t quadratic_dist(edge_segment_t* e, vec2 origin, double* param)
{
    ...
    *param = -vec2_mul_inner(qa, ab) / vec2_mul_inner(ab, ab);
    ...
    if (*param >= 0 && *param <= 1) return (signed_distance_t){min_distance, 0};
    ...
    ex_metrics_t* metrics)
{
    ...
    int ix0_i, iy0_i, ix1_i, iy1_i;
    float xoff = .5, yoff = .5;
    stbtt_GetGlyphBox(font, glyph_index, &ix0_i, &iy0_i, &ix1_i, &iy1_i);
    float ix0 = ix0_i;
    float iy0 = iy0_i;
    float ix1 = ix1_i;
    float iy1 = iy1_i;
    ...
    if (autoscale)
    {
        // Recompute a new scale, so that the glyph tightly fits in the bitmap,
        // while still having the same margin, and scaling the padding.
        // (Because the padding is used to display an outline, it must scale
        // along with the glyph.)

        float available_width = (w - 2 * margin);
        float available_height = (h - 2 * margin);
        float glyph_width = ix1 - ix0;
        float glyph_height = iy1 - iy0;
        float scaled_glyph_width = glyph_width * scale;
        float scaled_glyph_height = glyph_height * scale;

        // Try to fit the glyph on both axes, take the smallest scale so that
        // it fits on both.
        float scale_ratio_ww = available_width / (scaled_glyph_width + 2 * padding);
        float scale_ratio_wh = available_height / (scaled_glyph_width + 2 * padding);
        float scale_ratio_hw = available_width / (scaled_glyph_height + 2 * padding);
        float scale_ratio_hh = available_height / (scaled_glyph_height + 2 * padding);

        scale *= MIN(MIN(scale_ratio_ww, scale_ratio_wh), MIN(scale_ratio_hw, scale_ratio_hh));
    }

    // get left offset and advance
    int left_bearing_i, advance_i;
    stbtt_GetGlyphHMetrics(font, glyph_index, &advance_i, &left_bearing_i);
    float left_bearing = (float)left_bearing_i * scale;
    float advance = advance_i;

    // calculate offset for centering glyph on bitmap
    float translate_x = ((float)w / 2) - ((ix1 - ix0) * scale) / 2 - left_bearing;
    float translate_y = ((float)h / 2) - ((iy1 - iy0) * scale) / 2 - iy0 * scale;
    ...
    if (metrics)
    {
        ...
        metrics->iy0 = iy0;
        metrics->iy1 = iy1;
        metrics->translate_x = (translate_x / scale) * original_scale;
        metrics->translate_y = (translate_y / scale) * original_scale;
        metrics->scale = scale;
    }

    stbtt_vertex* verts;
    int num_verts = stbtt_GetGlyphShape(font, glyph_index, &verts);

    // figure out how many contours exist
    int contour_count = 0;
    for (int i = 0; i < num_verts; i++)
    {
        if (verts[i].type == STBTT_vmove) contour_count++;
    }
    ...
    // Scale the values of the SDF so that the length of the 0 to 1 distance
    // is 1 em. This allows specifying the outline width in ems.
    float em_scale = stbtt_ScaleForMappingEmToPixels(font, h - 2 * (margin + padding));
    double cscale = 1.0 / em_scale;
    ...
    for (int i = 0; i < contour_count; ++i)
    {
        if (contour_data[i].edge_count > 0)
        {
            ...
            for (int j = 0; j < contour_data[i].edge_count; ++j, ++index)
            {
                ...
                if (is_corner(prev_dir, dir, crossthreshold)) corners[corner_index++] = index;
        ...
        else if (corner_index == 1)
        {
            ...
            else if (contour_data[i].edge_count >= 1)
            {
                ...
                edge_split(
                    &contour_data[i].edges[0], parts[0 + 3 * corner], parts[1 + 3 * corner],
                    parts[2 + 3 * corner]);
                if (contour_data[i].edge_count >= 2)
                {
                    edge_split(
                        &contour_data[i].edges[1], parts[3 - 3 * corner], parts[4 - 3 * corner],
                        parts[5 - 3 * corner]);
    ...
    for (int y = 0; y < h; ++y)
    {
        ...
        for (int x = 0; x < w; ++x)
        {
            vec2 p = {(x + xoff - translate_x) / (scale * cscale),
                      (y + yoff - translate_y) / (scale * cscale)};
            ...
            for (int j = 0; j < contour_count; ++j)
            {
                ...
                if (signed_compare(r.min_distance, sr.min_distance)) sr = r;
                if (signed_compare(g.min_distance, sg.min_distance)) sg = g;
                if (signed_compare(b.min_distance, sb.min_distance)) sb = b;
                ...
                if (r.near_edge) dist_to_pseudo(&r.min_distance, p, r.near_param, r.near_edge);
                if (g.near_edge) dist_to_pseudo(&g.min_distance, p, g.near_param, g.near_edge);
                if (b.near_edge) dist_to_pseudo(&b.min_distance, p, b.near_param, b.near_edge);
            ...
            if (sr.near_edge) dist_to_pseudo(&sr.min_distance, p, sr.near_param, sr.near_edge);
            if (sg.near_edge) dist_to_pseudo(&sg.min_distance, p, sg.near_param, sg.near_edge);
            if (sb.near_edge) dist_to_pseudo(&sb.min_distance, p, sb.near_param, sb.near_edge);
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            if ((x > 0 && pixel_clash(P(x, y, w, bitmap), P(MAX(x - 1, 0), y, w, bitmap), tx))
                || (x < w - 1
                    && pixel_clash(P(x, y, w, bitmap), P(MIN(x + 1, w - 1), y, w, bitmap), tx))
                || (y > 0 && pixel_clash(P(x, y, w, bitmap), P(x, MAX(y - 1, 0), w, bitmap), ty))
                || (y < h - 1
                    && pixel_clash(P(x, y, w, bitmap), P(x, MIN(y + 1, h - 1), w, bitmap), ty)))
            {
                clashes[cindex].x = x;
                clashes[cindex++].y = y;
            }
    ...
    ex_metrics_t* metrics)
{
    ...
    if (ex_msdf_glyph_mem(font, glyph_index, font_scale, w, h, margin, padding, bitmap, autoscale, metrics) != 1)
    {
        free(bitmap);
        return NULL;
    }
