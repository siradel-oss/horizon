...
To update the `MODULE.bazel.lock` file, run `bazel mod deps --lockfile_mode=refresh`. This should be done after updating npm or Python dependencies (see below). This can also be used to fix merge conflicts in that lockfile.
...
{
    "dependencies": {
        "@siradel-oss/horizon-protocol": "workspace:*"
    }
}
```
