...
* `hrz`: The Horizon engine, bindings, and closely related tooling.
    ...
    * `doc`: User documentation and gallery.
        * `common`: Utilities shared by the doc and the gallery.
    * `fnd`: Utilities used my multiple modules. Only non-usercase-specific dependencies are allowed.
    * `generators`: Code generator for the API, protocol, etc., form the protocol.
...
* `tools`: Development or build tooling.
    ...
    * `dependencies`: Tooling to manage external dependencies.
