...
To publish code on the open-source repository using `//ci/oss_publish:publish`, you must either:

- **(Recommended)** Authenticate as the "Copysira" GitHub App. For this you need the GitHub App private key (as a PEM file). Ask your teammates how to obtain it. Then use the `--ghapp_pk_pem` option of `//ci/oss_publish:publish` to use it.
...
- Otherwise, try synchronizing `my_base` with by running `//ci/oss_publish:publish`, then retry the step above.
...
We have sometimes noticed issues when pushing changes from Copybara to the public repository in the form of errors 55 in Curl. This seems to be related to the size of commits to push. Manually running `//ci/oss_publish:publish` from another machine has fixed the issue, but there seem to exist other (untested) options, such as adjusting the `http.postBuffer` git option.
...
The license must be applied to all source files by the developer (it is checked during CI). This is done by running the `//ci/oss_publish:license` Bazel target. Use the `--check` flag for a dry run.
