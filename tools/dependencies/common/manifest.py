...
class ManifestEntry:
    ...
    def artifact_names(self) -> list[str]:
        if self.is_platform_dependent():
            return [
                f"{self.name}_{PLATFORM_NAME[p]}" for p in self.supported_platforms()
            ]
        else:
            return [self.name]

    def build_package(
        self, dst_dir_url: str, platform: Platform
    ) -> tuple[str, lockfile.LockEntry]:
...
class HttpArchiveManifestEntry(ManifestEntry):
    ...
    def build_package(
        ...
        package_name = (
            f"{self.name}_{PLATFORM_NAME[platform]}"
            if self.is_platform_dependent()
            else self.name
        )
        return package_name, lockfile.HttpArchiveLockEntry(
            [self.url, new_url],
            dgst,
            prefix=self.prefix,
...
class GithubManifestEntry(HttpArchiveManifestEntry):
    def __init__(
        ...
        super().__init__(
            name, url, prefix=prefix, patches=patches, build_file=build_file
        )
...
class HttpFileManifestEntry(ManifestEntry):
    def __init__(
        self, name: str, url: str, executable: bool, platforms: list[Platform]
    ):
        super().__init__(name, platforms)
        self.url = url
    ...
    def build_package(
        ...
        return package_name, lockfile.HttpFileLockEntry(
            [self.url, new_url], dgst, executable=self.executable
        )
...
class ExternalManifestEntry(ManifestEntry):
    ...
    def build_local_archive(self, platform: Platform) -> Path:
        ...
        file_name = (
            f"prebuilt_{short_platform}.{version_hash}.tar.gz"
            if self.is_platform_dependent()
            else f"prebuilt_{version_hash}.tar.gz"
        )
    ...
    def build_package(
        ...
        mirror_filename = (
            f"{self.name}-{short_platform}-{self.version}.tar.gz"
            if self.is_platform_dependent()
            else f"{self.name}-{self.version}.tar.gz"
        )
        mirror_url = upload_file_with_digest(
            local_archive, dgst, dst_dir_url, mirror_filename
        )

        return package_name, lockfile.LocalArchiveLockEntry(
            bazel_archive_label,
            build_file=bazel_build_file_label,
...
class Manifest:
    ...
    def build_package(
        self, name: str, platform: Platform
    ) -> tuple[str, lockfile.LockEntry]:
