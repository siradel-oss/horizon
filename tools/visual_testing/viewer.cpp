...
class Viewer
{
    ...
    ExitCode load_mapbox_style(const char* mapbox_style_filename)
    {
        ...
        input.set_style_json(std::string_view{buffer.data(), buffer.size()});
...
public:
    void wait_for_message(
        hrz_proto::TypedMessage::PayloadCase msg_type,
        uint64_t timeout_ms,
        ...
        uint32_t log_filter_level)
    {
        ...
        {
            // @Safety Taking .data() of the string_view returned by Name() is safe here because the
            // string_view is pointing to a static null-terminated string.
            printf(
                "Couldn't initialize Horizon backend: %s\n",
                hrz_proto::ViewerInitStatus_Name(backend->init_status()).data());
            backend->cleanup();
            wsi_cleanup();
            return ExitCode::FailedInitialization;
        }
...

        {
            ...
            wait_for_message(hrz_proto::TypedMessage::kViewerReady, 0, &msg);
        ...
        if (status == Status::TakeCapture)
        {
...

            {
                ...
                wait_for_message(hrz_proto::TypedMessage::kFrameCapture, 30000, &msg);
            ...
            if (capture.width() > 0)
            {
                ...
                stbi_write_png(
                    ...
                    capture.data().data(), capture.width() * 4 * sizeof(uint8_t));
            }
        }
