    ...
    const rapidjson::Value& source)
{
    ...
    else
    {
        ...
        std::string encoded = "data:;base64,";
        hrz::str::encode_base64(
            std::as_bytes(std::span<const char>(geojson_str, buffer.GetSize())), &encoded);
        provider->set_url(std::move(encoded));
    }
    ...
    PropertyAdder& properties)
{
    ...
    if (has_fill_pattern)
    {
        ...
        pattern->set_image_url(sprite_image_url);
    ...
    PropertyAdder& properties)
{
    ...
    auto use_default_palette = [&]()
    {
        ...
        {
            auto stop = palette->add_color_stops();
    ...
    if (!paint_node.IsNull())
    {
        ...
        if (heatmap_color_root_index != NO_NODE)
        {
            if (heatmap_color_expr.palettes.size() == 1
                && heatmap_color_expr.palettes.front().palette_type_case()
                    == hrz_proto::Palette::PaletteTypeCase::kNumeric)
            {
                ...
                for (auto& stop : *heatmap->mutable_numeric_palette()->mutable_color_stops())
                {
    ...
    PropertyAdder& properties)
{
    ...
    else // "map"
    {
        ...
        {
            // Must be static because "Value"s have the same lifetime as tje JSON structure.
            static constexpr std::string_view default_anchor_angle_attribute_name = "anchor_angle";

            if (layer_anchor_angle_attribute_name->empty())
            {
                *layer_anchor_angle_attribute_name =
                    std::string(default_anchor_angle_attribute_name);
            }

            auto anchor_angle_name =
                expr.add_literal(Value::from(default_anchor_angle_attribute_name));
    ...
    PropertyAdder& properties)
{
    ...
    if (has_icon)
    {
        ...
        image->set_url(sprite_image_url);
        ...
        if (has_text)
        {
            hrz_proto::BoxFitAxes fit_axes = hrz_proto::BoxFitAxes::BOX_FIT_AXES_NONE;
    ...
    MapboxVectorReprType repr_type)
{
    ...
    if (it == ctx->vector_source_hashes_to_data.end())
    {
        ...
        if (source->provider_type_case()
            == hrz_proto::VectorDataSource::ProviderTypeCase::kTilejsonDataProvider)
        {
            source->mutable_tilejson_data_provider()->set_layer_name(source_layer_name);
        }
        else if (
            source->provider_type_case()
            == hrz_proto::VectorDataSource::ProviderTypeCase::kPmtilesDataProvider)
        {
    ...
    for (auto& layer : vector_source->users)
    {
        ...
        for (size_t i = 0; i < (size_t)visibility_constraints.constraints_size(); ++i)
        {
            ...
            if (constraint.constraint_type_case() != ref.constraint_type_case())
            {
                matching_constraints = false;
                break;
            }

            if (constraint.constraint_type_case()
                == hrz_proto::LayerVisibilityConstraint::ConstraintTypeCase::kAltitude)
            {
                ...
                constraint.constraint_type_case()
                == hrz_proto::LayerVisibilityConstraint::ConstraintTypeCase::kBounds)
            {
    ...
    // different Mapbox sources or layers.
    {
        ...
        switch (repr_type)
        {
            ...
            case MapboxVectorReprType::Symbol:
            {
                ...
                create_symbol_repr(
                    ...
                    repr->mutable_symbol(), &anchor_angle_attribute_name, &ctx->next_symbol_z_index,
                    expr, property_adder);
                if (!anchor_angle_attribute_name.empty())
                {
                    style->set_anchor_angle_attribute_name(std::move(anchor_angle_attribute_name));
                }
                break;
            }
            default:
        ...
        if (first_new_representation_index < style->representations_size())
        {
            ...
            std::string styling_script{style->styling_script()};
            generate_representations_script(
                ...
                styling_script);
            style->set_styling_script(std::move(styling_script));
        }
    }
