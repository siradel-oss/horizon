...
inline std::optional<MapboxPropertyType> mapbox_property_type_array_to_component_type(
...
template<typename T>
T get_value(const Value& value);

template<>
constexpr uint64_t get_value<uint64_t>(const Value& value)
{
    return value.u64;
}

template<>
constexpr double get_value<double>(const Value& value)
{
    return value.f64;
}

template<typename T>
void set_value(Value& value, T v);

template<>
constexpr void set_value<uint64_t>(Value& value, uint64_t v)
{
    value.u64 = v;
}

template<>
constexpr void set_value<double>(Value& value, double v)
{
    value.f64 = v;
}
...
template<>
constexpr Value::Type get_value_type<double>()
{
