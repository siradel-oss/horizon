...
TEST_F(MapboxTranslation, parse_name)
{
    ...
    EXPECT_EQ(scene_dump.name(), "test");
}
...
struct RasterSourceConfig
{
    ...
    void test_output(const hrz_proto::LayerDump& layer)
    {
        ...
        EXPECT_EQ(
            ...
            layer.imagery_raster().raster().provider().provider_type_case());
        EXPECT_EQ(tiles, layer.imagery_raster().raster().provider().tiled().url_pattern());

        // There is no way of enforcing a local or global tiling scheme in Mapbox (?)
        EXPECT_EQ(
            hrz_proto::TilingSchemeParams::kGlobalTiling,
            layer.imagery_raster().raster().provider().tiled().tiling_scheme().scheme_type_case());
...
struct RasterLayerConfig
{
    ...
    void test_output(const hrz_proto::LayerDump& layer)
    {
        ...
        {
            EXPECT_TRUE(constraint.has_altitude());
...
TEST_F(MapboxTranslation, parse_geojson)
{
    ...
    for (const auto& layer : scene_dump.layers())
    {
        if (layer.has_vector_data())
        {
            ...
            EXPECT_EQ(
                source.provider_type_case(),
                hrz_proto::VectorDataSource::ProviderTypeCase::kUntiledDataProvider);
            EXPECT_EQ(source.has_geometry(), true);
            EXPECT_EQ(source.attributes_size(), 0);

            const auto& provider = source.untiled_data_provider();
            EXPECT_EQ(
                provider.url(), "https://redacted.localhost/assets/geojson/france_regions.geojson");
...
TEST_F(MapboxTranslation, parse_vector_source)
{
    ...
    for (const auto& layer : scene_dump.layers())
    {
        if (layer.has_vector_data())
        {
            ...
            EXPECT_EQ(
                src.provider_type_case(),
                hrz_proto::VectorDataSource::ProviderTypeCase::kTiledDataProvider);
            EXPECT_EQ(
                src.tiled_data_provider().url_pattern(),
                "https://redacted.localhost/assets/mvt/HK/{z}/{x}/{-y}.mvt");
            EXPECT_EQ(src.tiled_data_provider().layer_name(), "HK_SAMPLE_3857");
            EXPECT_EQ(
        ...
        else if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(c1.constraint_type_case(), hrz_proto::LayerVisibilityConstraint::kAltitude);
            EXPECT_EQ(
            ...
            EXPECT_EQ(c2.constraint_type_case(), hrz_proto::LayerVisibilityConstraint::kAltitude);
            EXPECT_EQ(
...
TEST_F(MapboxTranslation, parse_vector_shared_source)
{
    ...
    for (const auto& layer : scene_dump.layers())
    {
        if (layer.has_vector_data())
        {
            ...
            if (vdl.id() == 1)
            {
                EXPECT_EQ(vdl.sources(0).tiled_data_provider().layer_name(), "HK_SAMPLE_3857");
            }
            else if (vdl.id() == 2)
            {
                EXPECT_EQ(vdl.sources(0).tiled_data_provider().layer_name(), "HK_SAMPLE_4326");
            }
            else
            {
...
TEST_F(MapboxTranslation, parse_vector_fill_extrusion_layer)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                vtl.style().representations(0).repr_type_case(),
                hrz_proto::VectorRepr::ReprTypeCase::kExtrudedGeometry);
        ...
        else if (layer.has_vector_data())
        {
            ...
            EXPECT_EQ(
                vdl.sources(0).tilejson_data_provider().url(),
                "https://redacted.localhost/assets/mvt/HK/tiles.json");
            EXPECT_EQ(vdl.sources(0).tilejson_data_provider().layer_name(), "HK_SAMPLE_3857");
...
TEST_F(MapboxTranslation, parse_vector_fill_layer)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                vtl.style().representations(0).repr_type_case(),
                hrz_proto::VectorRepr::ReprTypeCase::kFlatOverlayPolygon);
        ...
        if (layer.has_vector_tiles())
        {
            ...
            for (const auto& repr : vtl.style().representations())
            {
                ...
                else if (repr.name() == "outline_hrz_outline")
                {
                    EXPECT_EQ(
                        repr.repr_type_case(),
                        hrz_proto::VectorRepr::ReprTypeCase::kFlatOverlayPolyline);
        ...
        if (layer.has_vector_tiles())
        {
            ...
            for (const auto& repr : vtl.style().representations())
            {
                if (repr.name() == "pattern")
                {
                    ...
                    EXPECT_EQ(flat_overlay.pattern().sprite_name().default_value(), "SableHumide");
...
TEST_F(MapboxTranslation, parse_vector_circle)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                vtl.style().representations(0).repr_type_case(),
                hrz_proto::VectorRepr::ReprTypeCase::kSymbol);

            const auto& symbol = vtl.style().representations(0).symbol();

            EXPECT_EQ(
                symbol.root_element().element_type_case(),
            ...
            EXPECT_EQ(
                anchor.child().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kSizedBox);
            ...
            EXPECT_EQ(
                sized_box.child().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kDecoratedShape);
...
TEST_F(MapboxTranslation, parse_vector_circle_translate_anchor)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            for (const auto& repr : vtl.style().representations())
            {
                EXPECT_EQ(repr.repr_type_case(), hrz_proto::VectorRepr::ReprTypeCase::kSymbol);
                ...
                if (repr.name() == "map_anchor")
                {
                    ...
                    EXPECT_NE(
                        anchor.child().element_type_case(),
                        hrz_proto::SymbolElement::ElementTypeCase::kTransform);
                }
                else if (repr.name() == "viewport_anchor")
                {
                    ...
                    EXPECT_EQ(
                        component.component_type_case(),
                        hrz_proto::TransformSymbolComponent::ComponentTypeCase::kTranslation);
...
TEST_F(MapboxTranslation, parse_vector_symbol_sprite)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                anchor.child().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kPadding);
            const auto& padding = anchor.child().padding();

            EXPECT_EQ(
                padding.child().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kImage);
...
TEST_F(MapboxTranslation, parse_vector_symbol_icon)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                transform_component.component_type_case(),
                hrz_proto::TransformSymbolComponent::ComponentTypeCase::kTranslation);
            ...
            EXPECT_EQ(
                transform.child().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kImage);
            ...
            EXPECT_EQ(
                image.url(), "https://redacted.localhost/assets/sprites/sprites_mapbox/tree.png");
...
TEST_F(MapboxTranslation, parse_vector_symbol_text)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                transform.child().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kText);
            ...
            EXPECT_EQ(text.text().default_value(), "sample\ntext");
            EXPECT_EQ(
                text.font_url(), "https://redacted.localhost/assets/fonts/Roboto/Roboto-Regular.ttf");
...
TEST_F(MapboxTranslation, parse_vector_symbol_icon_text)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                symbol.root_element().element_type_case(),
                hrz_proto::SymbolElement::ElementTypeCase::kStack);
...
TEST_F(MapboxTranslation, parse_vector_symbol_icon_text_fit)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            for (const auto& stack_child : stack.children())
            {
                if (stack_child.element_type_case()
                    == hrz_proto::SymbolElement::ElementTypeCase::kStackExpand)
                {
                    const auto& stack_expand = stack_child.stack_expand();

                    EXPECT_EQ(
                        stack_expand.child().element_type_case(),
                        hrz_proto::SymbolElement::ElementTypeCase::kAnchor);
                    ...
                    == hrz_proto::SymbolElement::ElementTypeCase::kPadding)
                {
                    ...
                    EXPECT_EQ(
                        fit_padding.child().element_type_case(),
                        hrz_proto::SymbolElement::ElementTypeCase::kAnchor);
...
TEST_F(MapboxTranslation, parse_vector_heatmap_interpolate)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_EQ(
                vtl.style().representations(0).repr_type_case(),
                hrz_proto::VectorRepr::ReprTypeCase::kHeatmap);
            ...
            EXPECT_EQ(palette.color_stops_size(), 5);

            {
                EXPECT_FLOAT_EQ(palette.color_stops(0).value(), 0.0F);
                const auto& first_color = palette.color_stops(0).first_color();
                ...
                const auto& second_color = palette.color_stops(0).second_color();
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(1).value(), 0.25F);
                const auto& first_color = palette.color_stops(1).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.a(), 0.5F);
                const auto& second_color = palette.color_stops(1).second_color();
                EXPECT_FLOAT_EQ(second_color.r(), 1.0F);
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(2).value(), 0.5F);
                const auto& first_color = palette.color_stops(2).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.a(), 0.5F);
                const auto& second_color = palette.color_stops(2).second_color();
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(3).value(), 0.75F);
                const auto& first_color = palette.color_stops(3).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.a(), 0.5F);
                const auto& second_color = palette.color_stops(3).second_color();
                EXPECT_FLOAT_EQ(second_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(second_color.g(), 1.0F);
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(4).value(), 1.0F);
                const auto& first_color = palette.color_stops(4).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.a(), 0.5F);
                const auto& second_color = palette.color_stops(4).second_color();
                EXPECT_FLOAT_EQ(second_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(second_color.g(), 0.0F);
                EXPECT_FLOAT_EQ(second_color.b(), 1.0F);
                EXPECT_FLOAT_EQ(second_color.a(), 0.5F);
        ...
        else if (layer.has_vector_data())
        {
            ...
            const auto& provider = source.untiled_data_provider();
            EXPECT_EQ(provider.url(), "https://redacted.localhost/assets/geojson/earthquakes.geojson");
            EXPECT_EQ(provider.format(), hrz_proto::VectorDataFormat::GEOJSON_VECTOR_DATA);

            const auto& attribute = source.attributes(0);
            EXPECT_EQ(attribute.source_name(), "mag");
        }
    };
}
...
TEST_F(MapboxTranslation, parse_vector_heatmap_step)
{
    ...
    for (int i = 0; i < scene_dump.layers_size(); i++)
    {
        ...
        if (layer.has_vector_tiles())
        {
            ...
            EXPECT_FALSE(palette.has_nan_color());
            EXPECT_EQ(palette.color_stops_size(), 4);

            {
                EXPECT_FLOAT_EQ(palette.color_stops(0).value(), 0.25F);
                const auto& first_color = palette.color_stops(0).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 0.0F);
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(1).value(), 0.5F);
                const auto& first_color = palette.color_stops(1).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 0.0F);
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(2).value(), 0.75F);
                const auto& first_color = palette.color_stops(2).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 0.0F);
            ...
            {
                EXPECT_FLOAT_EQ(palette.color_stops(3).value(), 1.0F);
                const auto& first_color = palette.color_stops(3).first_color();
                EXPECT_FLOAT_EQ(first_color.r(), 0.0F);
                EXPECT_FLOAT_EQ(first_color.g(), 1.0F);
                EXPECT_FLOAT_EQ(first_color.b(), 1.0F);
