...
def parse(input_file_name):
    ...
    for proto_file in root.iter("file"):
        ...
        for s in proto_file.iter("service"):
            ...
            service["documentation"] = remove_leading_spaces(
                s.findtext("documentation", default="")
            )
            ...
            for m in s.iter("method"):
                ...
                method["documentation"] = remove_leading_spaces(
                    m.findtext("documentation", default="")
                )
                method["deprecated"] = (
                    True if m.find("deprecated").text == "true" else False
                )
        ...
        for e in proto_file.iter("enum"):
            ...
            enum["documentation"] = remove_leading_spaces(
                e.findtext("documentation", default="")
            )
            enum["expose_to_style"] = (
                True if e.find("expose_to_style").text == "true" else False
            )
            enum["values"] = []
            ...
            for v in e.iter("value"):
                ...
                val["deprecated"] = (
                    True if v.find("deprecated").text == "true" else False
                )
                val["documentation"] = remove_leading_spaces(
                    v.find("documentation").text
                )
                val["params_field_name"] = v.findtext("params_field_name", default="")
                val["response_field_name"] = v.findtext(
                    "response_field_name", default=""
                )
        ...
        for m in proto_file.iter("message"):
            ...
            msg["package"] = package
            msg["documentation"] = remove_leading_spaces(
                m.findtext("documentation", default="")
            )
            msg["fields"] = []
            ...
            if path_root_node != None:
                root_field = path_root_message.find(
                    "field[union='kind'][name='%s']" % path_root_node.text
                )
                msg["path_root"] = path_root_node.text
                msg["path_root_type"] = root_field.find("type").text
                root_type_is_enum = (
                    root.find("file/enum[full_name='%s']" % msg["path_root_type"])
                    != None
                )
            ...
            for f in m.iter("field"):
                ...
                field["deprecated"] = (
                    True if f.find("deprecated").text == "true" else False
                )
                field["optional"] = True if f.find("optional").text == "true" else False
                field["documentation"] = remove_leading_spaces(
                    f.findtext("documentation", default="")
                )
