...
static const double SPLIT_F = 65536.0;
...
static double vertical_to_horizontal_fov(double vfov_rad, double aspect_ratio)
{
...
static double normalize_angle_around_zero(double angle)
{
    return angle - std::floor((angle + lm::PI) / (2 * lm::PI)) * 2 * lm::PI;
}

// Angles must be in [-pi;pi[
static double distance_between_normalized_angles(double from, double to)
{
    double a = to - from;
...
static double clamp_angle(double angle, double min, double max)
{
    PrecomputedClampAngle c(min, max);
