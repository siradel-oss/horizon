...
TEST(MessageBuffer, parsing)
{
    ...
    auto sample = arena.Make<hrz_monitoring_proto::Sample>();
    sample->set_entry(20);
    sample->set_exit(60);
    ...
    auto message1 = arena.Make<hrz_monitoring_proto::MonitoringMessage>();
    message1->set_allocated_sample(sample.get());

    auto metric = arena.Make<hrz_monitoring_proto::Metric>();
    ...
    auto message2 = arena.Make<hrz_monitoring_proto::MonitoringMessage>();
    message2->set_allocated_metric(metric.get());

    auto histogram = arena.Make<hrz_monitoring_proto::Metric>();
    ...
    auto message3 = arena.Make<hrz_monitoring_proto::MonitoringMessage>();
    message3->set_allocated_metric(histogram.get());

    write_message(&coded_ostream, message1.get());
    write_message(&coded_ostream, message2.get());
    write_message(&coded_ostream, message3.get());
    ...
    EXPECT_EQ(parsed_messages[1].metric().name(), "TestValue");
    ...
    EXPECT_EQ(parsed_messages[2].metric().name(), "TestHistogram");
...
TEST(MessageBuffer, serializing)
{
    ...
    auto messages = arena.Make<hrz_monitoring_proto::MonitoringMessages>();
