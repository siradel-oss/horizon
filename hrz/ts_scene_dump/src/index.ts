...
export interface LoadedScene {
    name: string;
    layers: HrzProtocol.Layer[];
    viewpoints: HrzProtocol.ViewpointDump[];
}

export async function loadSceneObjAsync(
    api: HrzApi.AsyncApi,
    dump: HrzProtocol.SceneDump.$Properties,
    cameraAnimationOptions: HrzProtocol.CameraAnimationOptions.$Properties,
    ...
    let loadedLayers: LoadedScene = {
        name: dump.name ?? "",
        layers: layers.layers.map((l) => HrzProtocol.Layer.create(l)),
        viewpoints: (dump.viewpoints ?? []).map((v) => HrzProtocol.ViewpointDump.create(v)),
    };
...
export async function dumpSceneObjAsync(
    api: HrzApi.AsyncApi,
    sceneName: string = "",
    viewpoints: HrzProtocol.ViewpointDump.$Properties[] = []
