...
template<typename T> struct ToOwnedPrimitiveType { using Type = T; };
template<> struct ToOwnedPrimitiveType<std::string_view> { using Type = std::string; };
...
public:
    using Owned = typename internal::ToOwnedPrimitiveType<PrimitiveType>::Type;
    ...
    Owned get() &&
    {
        ...
        WrapperType wrapper;
        (void)wrapper.ParseFromString(raw);
        return Owned{wrapper.value()};
    }
};
