...
def gather_client_messages(protocol: HrzProtocol) -> list[HrzApiClientMessage]:
    ...
    if client_typed_message is None:
        raise Exception("Failed to find TypedMessage in protocol")
    ...
    for field in client_typed_message.fields:
        ...
        if message is None:
            raise Exception(
                "Failed to find message for client message type %s" % field.name
            )
