...
public:
    DesktopHttpLoader(std::string_view user_agent, std::string_view http_referrer)
    {
        // CURL_GLOBAL_WIN32:
        ...
        for (size_t i = 0; i < MAX_AVAILABLE_HANDLES; ++i)
        {
            ...
            auto user_agent_str = std::string{user_agent};
            auto http_referrer_str = std::string{http_referrer};
            ...
            curl_easy_setopt(handle, CURLOPT_USERAGENT, user_agent_str.c_str());
            curl_easy_setopt(handle, CURLOPT_ERRORBUFFER, _slots[i].error_buffer.get());
            curl_easy_setopt(handle, CURLOPT_REFERER, http_referrer_str.c_str());
