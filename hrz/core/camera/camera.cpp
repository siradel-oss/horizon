...
{
    // @Todo(977) Source from the API once available (CameraAnimationOptions).
    hrz::clock::ClockType _clock_type = hrz::clock::kWallClock;
...
class CameraImpl : public Camera
{
    ...
    double _last_work_time_wall = 0;
    double _last_work_time_sim = 0;

    hrz_proto::CameraIndex _index;
    bool _camera_settings_changed = true;

    bool _last_frame_idle = true;
    double _last_movement_time_wall = 0;
...
public:
    CameraImpl(
        ...
        _min_height_above_terrain(min_height_above_terrain),
        _terrain_collision_inertia(terrain_collision_inertia),
        _last_work_time_wall(hrz::clock::CurrentFrameWallTime.s),
        _last_work_time_sim(hrz::clock::CurrentFrameSimTime.s),
        _index(index)
    {
        ...
        override
    {
        ...
        const double now_wall = hrz::clock::CurrentFrameWallTime.s;
        const double now_sim = hrz::clock::CurrentFrameSimTime.s;

        const double dt_wall = now_wall - std::exchange(_last_work_time_wall, now_wall);
        const double dt_sim = now_sim - std::exchange(_last_work_time_sim, now_sim);

        lm::ddual_quat new_pose = _manipulator->work(
            TimeStep{dt_wall, dt_sim}, _baked_info.fovy, viewport, height_above_terrain, picking,
            notifications_cb);

        const bool has_moved = std::exchange(_pose, new_pose) != new_pose;
        if (has_moved)
        {
            if (_last_frame_idle)
            {
            ...
            _last_movement_time_wall = now_wall;
            bake_info();
        }
        else if (!_last_frame_idle && now_wall - _last_movement_time_wall > kIdleDetectionDelayS)
        {
