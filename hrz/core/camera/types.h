...
struct ViewportInfo
{
    ...
    inline lm::vec2 subview_size() const { return lm::round(size * lm::size(subfrustum)); }
};
...
struct TimeStep
{
    ...
    constexpr double select(clock::ClockType type) const
    {
        return type == clock::kWallClock ? wall : sim;
    }
};
