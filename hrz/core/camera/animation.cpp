...
Animation make_arc_animation(const lm::ddual_quat& from_pose, const lm::ddual_quat& to_pose)
{
    ...
    if (lm::length(to_surface_ecef - from_surface_ecef) < 1e-3)
    {
...
lm::ddual_quat AnimationPlayer::advance(hrz::TimeStep dt, double height_above_terrain)
{
    const double dt_selected = dt.select(_clock_type);

    _t += dt_selected / _duration;
    if (_t < 1.0)
    {
