...
static constexpr double kDefaultTranslationDistance = 1;
...
class OrbitManipulator : public CameraManipulator
{
    ...
    public:
        TouchDragController(
            ...
            gestures::GestureId gesture_id,
            lm::vec2 position) :
            MoveAroundPlanetController(state, view_index, position, clock::kWallClock),
            _gesture_id(gesture_id),
    ...
    public:
        InstantMovementZoomStepController(
            ...
            // @Todo(977) source from the API once available (CameraMovement).
            DiscreteZoomController(
                state,
                state.config.energy_half_time.for_movements,
                hrz::clock::kWallClock),
    ...
    protected:
        ...
        DiscreteZoomPickedTargetController(
            ...
            hrz_proto::SceneViewIndex scene_view,
            clock::ClockType clock_type) :
            // Always wall clock: direct user action, never configurable.
            DiscreteZoomController(
                state,
                state.config.energy_half_time.for_user_controls,
                clock_type),
    ...
    public:
        MouseWheelController(const InitializedState& state, hrz_proto::SceneViewIndex scene_view) :
            DiscreteZoomPickedTargetController(state, scene_view, clock::kWallClock)
        {
        }
...
public:
        ...
        override
    {
        ...
        {
            _last_non_idle_time = hrz::clock::CurrentFrameSimTime.s;
        }
        ...
        if (state.config.correction.duration() > 0)
        {
            ...
            if (hrz::clock::CurrentFrameSimTime.s - _last_non_idle_time
