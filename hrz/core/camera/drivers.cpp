...
public:
    ...
    std::unique_ptr<ZoomDriver> recreate_with_pose(const lm::ddual_quat& pose) override
    {
        return ZoomDriver::create(
            ...
            _min_height_above_terrain, _terrain_collision_inertia, _clock_type);
    }
    ...
    lm::ddual_quat work(TimeStep dt, double height_above_terrain, bool should_keep_bearing) override
    {
        const double dt_selected = dt.select(_clock_type);
        ...
        Inertia inertia(_energy_half_time, dt_selected);
    ...
    hrz::clock::ClockType clock_type)
{
    return std::make_unique<ZoomBearingDriverImpl>(
        ...
        min_height_above_terrain, terrain_collision_inertia, clock_type);
}
    ...
    hrz::clock::ClockType clock_type)
{
    return std::make_unique<RotateAroundCenterDriverImpl>(
        ...
        terrain_collision_inertia, clock_type);
}
    ...
    lm::ddual_quat work(TimeStep dt, double height_above_terrain, bool should_keep_bearing) override
    {
        ...
        if (height_above_terrain < _min_height_above_terrain)
        {
            _pose = push_camera_position_above_terrain(
                ...
                _terrain_collision_inertia * kDragTerrainCollisionTimeFactor, dt_selected);
        }
...
public:
    ...
    lm::ddual_quat work(TimeStep dt, double height_above_terrain, bool /*should_keep_bearing*/)
        override
    {
        ...
        if (height_above_terrain < _min_height_above_terrain)
        {
            _pose =
                push_camera_position_above_terrain_with_target(
                    ...
                    _terrain_collision_inertia * kRotationTerrainCollisionTimeFactor, dt_selected)
                    .pose;
        }
    ...
    lm::ddual_quat work(TimeStep dt, double height_above_terrain, bool should_keep_bearing) override
    {
        ...
        if (height_above_terrain < _min_height_above_terrain)
        {
            _viewpoint.tilt = push_camera_position_above_terrain_with_target(
                                  _pose, height_above_terrain, _min_height_above_terrain, _target,
                                  _terrain_collision_inertia, dt_selected)
                                  .tilt;
            recompute_pose = true;
        }
...
public:
        ...
        const lm::ddual_quat& pose) override
    {
        return MaintainHeightAboveTerrainDriver::create(
            pose, _min_height_above_terrain, _terrain_collision_inertia, _clock_type);
    }
    ...
    lm::ddual_quat work(TimeStep dt, double height_above_terrain, bool should_keep_bearing) override
    {
        if (height_above_terrain < _min_height_above_terrain)
        {
            _pose = push_camera_position_above_terrain(
                _pose, height_above_terrain, _min_height_above_terrain,
                _terrain_collision_inertia * kDragTerrainCollisionTimeFactor,
                dt.select(_clock_type));
        }
    ...
    hrz::clock::ClockType clock_type)
{
    return std::make_unique<MaintainHeightAboveTerrainDriverImpl>(
        pose, min_height_above_terrain, terrain_collision_inertia, clock_type);
}
...
public:
    AnimationDriverImpl(
        ...
        double terrain_collision_inertia,
        hrz::clock::ClockType clock_type) :
        _player(from, to, options, min_height_above_terrain, terrain_collision_inertia, clock_type),
        _pose{from}
    {
    }
    ...
    hrz::clock::ClockType clock_type)
{
    return std::make_unique<AnimationDriverImpl>(
        from, to, options, min_height_above_terrain, terrain_collision_inertia, clock_type);
}
...
{
    return std::make_unique<AnimationDriverImpl>(
        animation, options, min_height_above_terrain, terrain_collision_inertia, clock_type);
}
