...
struct PlanetSurface
{
    ...
    // Bitset of the scene views that currently have DTM enabled.
    uint32_t dtm_enabled_views;
        ...
        std::span<PlanetGeometry*> geometries)
    {
        ...
        hrz::InlinedVector<std::span<const planet::RequestedTileCoords>, SCENE_VIEW_COUNT>
            requested_tile_lists;
        size_t requested_tiles_hash = 0;

        // Scene views that have DTM disabled must not cause DTM tiles to be
        // loaded, so they get their own list of requested tiles.
        hrz::InlinedVector<std::span<const planet::RequestedTileCoords>, SCENE_VIEW_COUNT>
            requested_dtm_tile_lists;
        size_t requested_dtm_tiles_hash = 0;
        uint32_t new_dtm_enabled_views = 0;
        ...
        for (size_t i = 0; i < geometries.size(); ++i)
        {
            ...
            if (planet::is_dtm_enabled(geometry))
            {
                new_dtm_enabled_views |= 1U << views_info[i].view;
                requested_dtm_tile_lists.push_back(tiles.tiles);
                requested_dtm_tiles_hash = hrz::hash_mix(requested_dtm_tiles_hash, tiles.hash);
            }
        }

        bool dtm_enabled_views_updated = new_dtm_enabled_views != dtm_enabled_views;
        dtm_enabled_views = new_dtm_enabled_views;

        if (dtm_enabled_views_updated)
        {
            // The set of scene views for which DTM is enabled has changed.
            // Make sure that even if the requested tiles are the same as
            // before, these tiles get actually requested.
            dtm_rasters.reset_requested_tiles_history();
        }

        if (requested_tiles_updated || dtm_enabled_views_updated)
        {
            dtm_rasters.update_requested_tiles(
                requested_dtm_tile_lists, requested_dtm_tiles_hash, al, js);
        }
...
std::optional<float> fast_query_elevation(PlanetSurface* planet, const GeoPosition2& position)
{
    assert(planet);

    auto* group = planet->dtm_rasters.get_group(0);

    hrz_proto::PickResults pick_results;
    group->pick(&planet->dtm_rasters, hrz::geo_to_ecef(position), std::nullopt, pick_results);

    for (int i = pick_results.results_size() - 1; i >= 0; --i)
    {
        const auto& raster_result = pick_results.results(i).raster();

        if (raster_result.has_number() && !raster_result.nodata())
        {
            return (float)raster_result.number();
        }
    }
