...
HttpHeaders build_headers(hrz_proto::HttpHeaderList header_list)
{
    ...
    header->set_name("Accept");
    header->set_value("application/vnd.quantized-mesh,application/octet-stream;q=0.9");
...
public:
    ...
    {
        return hrz_proto::RasterProvider::ProviderTypeCase::kCesiumTerrain;
    }
        ...
        fetcher.emplace(
            ...
            TileFetcher::MetricInfo{
                provider_request_tally_metric_name(
                    hrz_proto::RasterProvider::ProviderTypeCase::kCesiumTerrain),
                raw_url.c_str()
            });
...
public:
    ...
    {
        if (path.leaf() || !path.is_cesium_terrain())
        {
