...
public:
    ...
    // Forget the list of tile lists that have been requested recently, so that
    // the next call to `update_requested_tiles` is taken into account even if
    // the very same tiles have just been requested.
    void reset_requested_tiles_history() { _requested_tile_list_hashes.clear(); }
    ...
    // If no `included_rasters` list is provided, all rasters are picked.
    void pick(
        IRasterCollection* collection,
        const lm::dvec3& position,
        std::optional<std::span<const hrz_proto::LayerHandle>> included_rasters,
