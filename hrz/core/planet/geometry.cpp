...
public:
    ...
    void execute_timed(const my::RenderGraph::ExecutionContext& ctx) override
    {
        ...
        if (_last_feedback_size.x == 0 || _last_feedback_size.y == 0)
        {
            _scheduled = false;
            return;
        }
        ...
        if (_tree.to_render.size() > 0)
        {
            ...
            if (_dtm_enabled)
            {
                ...
                _height_precomputation.draw(
                    ...
                    {}, resources.planet_params, _tree.patch_bins,
                    _tree.precompute_bin_data.get_for_gpu());
            }
        }
    }
...
struct PlanetGeometry
{
    ...
    double last_feedback_start_ms = hrz::clock::CurrentFrameWallTime.ms;
    ...
    void draw(Render* render, const planet::GeometryResources& resources, bool cast_shadows)
    {
        ...
        {
            renderable->draw(
                render, resources, !is_opaque, dtm_enabled, model_scene_view_index, cast_shadows);
        }
    }
        ...
        double mipmap_bias)
    {
        ...
        if (model_updated)
        {
            ...
            dtm_enabled = !settings.disable_terrain_elevation();
            model_updated = false;
        }

        use_adaptive_resolution = hrz::get_flag(hrz::Flag::EnableTerrainAdaptiveResolution);
...
bool is_dtm_enabled(const PlanetGeometry* planet)
{
    assert(planet);
    return planet->dtm_enabled;
}
