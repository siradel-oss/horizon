    ...
    JobScheduler* js)
{
                          ...
                          const TileCoords& composed_coords)
    {
        ...
        if (auto image_format = raster->provider->get_image_format();
            ...
            || image_format == hrz_proto::ImageFormat::SIRADEL_LEGACY_F32
            || image_format == hrz_proto::ImageFormat::TERRARIUM
            || image_format == hrz_proto::ImageFormat::TERRAIN_RGB
            || raster->provider->get_raster_provider_type() == hrz_proto::RasterProvider::kUntiled
    ...
    hrz_proto::PickResults& pick_results)
{
    ...
    for (const auto& raster_pair : _rasters)
    {
        ...
        bool included = !included_rasters.has_value();

        if (!included)
        {
            for (const auto& layer : included_rasters.value())
            {
