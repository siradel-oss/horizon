...
{
    hrz::log::message((hrz::log::Severity)severity, {message, message_length}, location);
}
...
public:
    ...
    {
        const double now = hrz::clock::CurrentFrameWallTime.ms;
...
private:
    ...
    double _last_info_time_ms = hrz::clock::CurrentFrameWallTime.ms;
    double _last_highlight_time_ms = hrz::clock::CurrentFrameWallTime.ms;
};
...
public:
        ...
        uint32_t num_shaders_ready)
    {
        ...
        if (_fadeout && _num_frames_skipped <= NumFramesToSkipBeforeFadeout)
        {
            ...
            _fadeout_start_ms = hrz::clock::CurrentFrameWallTime.ms;
            _num_frames_skipped++;
        }

        const double elapsed = hrz::clock::CurrentFrameWallTime.ms - _fadeout_start_ms;
        ...
        render->draw(batch_info, _shader, _vertex_input, {&ubo_binding, 1}, {});
...
public:
        ...
        _platform(platform), _monitoring(options), _options(options), _key_interactions(options)
    {
        ...
        hrz::set_flag(
            hrz::Flag::EnableTerrainAdaptiveResolution,
            !options.disable_terrain_adaptive_resolution());
...
#undef HRZ_DEFINE_GRAPHICS_SETTING
        ...
        _assets_loader = hrz::assets_loader::create(
            _options.user_agent(), options.native_http_referrer(),
    ...
    void fast_query_elevation(
        const ::hrz_proto::ElevationRequest& request,
        ::hrz_proto::ElevationRequestResult& output)
    {
        auto res =
            hrz::scene::fast_query_elevation(_scene, hrz::from_proto(request.position()).latlon());
        output.set_has_value(res.has_value());
        output.set_value(res.value_or(0));
    }
    ...
    void do_attribution_message()
    {
        ...
        if (!_attribution_messages_enabled
            || hrz::clock::CurrentFrameWallTime.s - _last_attribution_message_time_s
                < kAttributionDelayS)
            return;

        hrz::scene::enqueue_attribution_message(_scene, _message_queue, &_last_attribution_hash);
        _last_attribution_message_time_s = hrz::clock::CurrentFrameWallTime.s;
    }
    ...
    void capture_and_return_frame()
    {
        ...
        std::string data_str(data.get(), _viewport_width * _viewport_height * 4);
        image.set_data(std::move(data_str));
...
public:
    ...
    void get_terrain_elevation(
        const ::hrz_proto::ElevationRequest& input,
        ::hrz_proto::ElevationRequestResult& output) override
    {
        core->fast_query_elevation(input, output);
    }
    ...
    void set_user_interactions_enabled(const ::hrz_proto::BoolValue& input, ::hrz_proto::Void&)
        override
    {
    ...
    void clear_http_cache(const ::hrz_proto::Void&, ::hrz_proto::Void&) override
    {
        core->clear_http_cache();
    }

    void set_attribution_enabled(const ::hrz_proto::BoolValue& input, ::hrz_proto::Void&) override
    {
...
public:
    void provide_vector_data(
        const ::hrz_proto::VectorDataRequestResponse& input,
    ...
    void invalidate_vector_data(
        const ::hrz_proto::VectorDataInvalidation& input,
        ::hrz_proto::Void&) override
    {
        hrz::scene::invalidate_client_vector_data(core->scene(), input);
    }

    void provide_asset_data(const ::hrz_proto::AssetRequestResponse& input, ::hrz_proto::Void&)
        override
    {
...
public:
    ...
    void get_oneof_case(const ::hrz_proto::Path& path, ::hrz_proto::UInt32Value& output) override
    {
        output.set_value(hrz::scene::get_oneof_case_model(core->scene(), path));
    }
};
...
{
public:
    void dump_scene(const ::hrz_proto::SceneDumpRequest& input, ::hrz_proto::SceneDump& output)
        override
    {
        hrz::scene::dump_scene(core->scene(), input, output);
    }

    void load_dump(const ::hrz_proto::SceneLoadRequest& input, ::hrz_proto::LayerArray& output)
        override
    {
...
class MapboxServiceImpl : public hrz_proto::IMapboxService
{
    void translate_scene(
        const ::hrz_proto::MapboxTranslationParams& input,
        ::hrz_proto::MapboxTranslationTicket& output) override
    {
    ...
    const char* canvas_selector)
{
    ...
    HRZ_LOG_INFO("Build date:     {}", hrz::BuildDate);
    HRZ_LOG_INFO("Build git hash: {}", hrz::BuildRevision);
    HRZ_LOG_INFO("Build host:     {}", hrz::BuildHost);
    ...
    HRZ_LOG_INFO("    HTTP referrer: {}", options.native_http_referrer());
#endif
...
uint32_t hrz_core_frame(void)
{
    ...
    hrz::clock::CurrentFrameWallTime = hrz::now_all_variants();
    hrz::clock::CurrentFrameSimTime = hrz::clock::CurrentFrameWallTime;

    const bool should_continue = core->frame();
