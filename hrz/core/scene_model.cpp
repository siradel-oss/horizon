...
struct ArenaMessage
{
    ...
    google::protobuf::Arena::Ptr<T> msg;
    uint64_t previous_used_bytes;

    ArenaMessage() :
        arena{new pb::Arena()}, msg{arena->Make<T>()}, previous_used_bytes{arena->SpaceUsed()}
    {
    }

    void defrag()
    {
        const uint64_t current_used_bytes = arena->SpaceUsed();
        if (current_used_bytes >= 4 * previous_used_bytes)
        {
            std::unique_ptr<pb::Arena> new_arena(new pb::Arena());
            auto new_msg = new_arena->Make<T>();
...
public:
    ...
    virtual uint32_t get_oneof_case(const hrz_proto::Path& path) const = 0;
...
public:
    ...
    uint32_t get_oneof_case(const hrz_proto::Path& path) const final
    {
        HRZ_SCOPED_SHARED_LOCK(_mutex);
        return hrz::scene_model::get_oneof_case_part_raw(*_model.msg, _path_to_span(path));
    }
...
public:
    ...
    void register_element(const hrz_proto::PathRoot& root) final
    {
        ...
        if (it == _models.end())
        {
            _models.emplace(std::piecewise_construct_t{}, std::forward_as_tuple(key), std::tuple{});
        }
        else
        {
    ...
    uint32_t get_oneof_case(const hrz_proto::Path& path) const final
    {
        ...
        if (it != _models.end())
        {
            return hrz::scene_model::get_oneof_case_part_raw(*it->second.msg, _path_to_span(path));
        }
        else
        {
            LOG_INVALID_ROOT("get_oneof_case", path.root());
            return 0;
        }
    }
...
SceneModel* create()
{
    ...
    model->last_defrag_ms = hrz::clock::CurrentFrameWallTime.ms;
...
uint32_t get_oneof_case(const SceneModel* model, const hrz_proto::Path& path)
{
    assert(model);
    auto it = model->models.find(path.root().kind_case());
    assert(
    ...
    if (it != model->models.end())
    {
        return it->second->get_oneof_case(path);
    }
    return 0;
}
