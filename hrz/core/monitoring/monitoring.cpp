    ...
    hrz_monitoring::MessageBuffer* mb) const
{
    ...
    PbArena arena;
    auto msgs = arena.Make<hrz_monitoring::MonitoringMessages>();
    auto* msg = msgs->add_messages();
    auto* snapshot = msg->mutable_gpu_resources();
    snapshot->set_timestamp(hrz::clock::CurrentFrameWallTime.us_s64);
    ...
    const char* window_name)
{
    if (mu_begin_window_ex(ctx, window_name, mu_rect(300, 200, 420, 370), MU_OPT_CLOSED))
    {
        ...
        if (mu_header(ctx, "Information"))
        {
            ...
            mu_text(ctx, "Build date");
            mu_text(ctx, hrz::BuildDate);

            mu_text(ctx, "Build git hash");
            mu_text(ctx, hrz::BuildRevision);

            mu_text(ctx, "Build host");
            mu_text(ctx, hrz::BuildHost);
