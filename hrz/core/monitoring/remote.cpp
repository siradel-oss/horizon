...
struct RemoteMonitoring : public ws::ClientHandler
{
    ...
    double _last_send_messages_ms = hrz::clock::CurrentFrameWallTime.ms;
    ...
    {
        auto msgs = _messages_arena.Make<hrz_monitoring::MonitoringMessages>();
    ...
    void send_messages(ClientMessageQueue* queue)
    {
        ...
        if (_send_to_message_queue)
        {
            ...
            message.set_data(std::string_view((const char*)data.data(), data.size()));
