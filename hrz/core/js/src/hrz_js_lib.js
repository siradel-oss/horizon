var LibraryHorizon = {
    ...
    // The atomic must be notified twice, including once one frame later.
    // Based on https://github.com/emscripten-core/emscripten/issues/20920#issuecomment-2565983153
    hrz_js_install_module_leak_fix__proxy: "sync",
    hrz_js_install_module_leak_fix__sig: "v",
    hrz_js_install_module_leak_fix__deps: ["$PThread"],
    hrz_js_install_module_leak_fix: function () {
        const pthread_ptr = _pthread_self();

        // The next line should be:
        //     const pthread_ptr_offset = {{{ getHeapOffset("pthread_ptr", "i32") }}};
        // But Prettier doesn't like the curly bracket macro syntax, as it's syntactically
        // invalid JavaScript when the result is allocated to a variable.
        // So instead the result of the macro expansion is manually copied here.
        // See https://github.com/emscripten-core/emscripten/blob/3.1.74/src/parseTools.mjs#L314
        const pthread_ptr_offset = pthread_ptr >>> 2;

        PThread.terminateAllThreadsOrig = PThread.terminateAllThreads;
        PThread.terminateAllThreads = function () {
            PThread.terminateAllThreadsOrig();

            console.log("Running module leak fix");

            checkMailbox = () => {};

            HEAP32[pthread_ptr_offset] = pthread_ptr;
            Atomics.notify(HEAP32, pthread_ptr_offset);
        ...
        console.log("Module leak fix installed");
    },
