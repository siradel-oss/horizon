...
/**
 * Gets the case of a oneof field in the scene model.
 */
uint32_t get_oneof_case(const SceneModel*, const hrz_proto::Path&);
...
public:
    ...
    CaseType get_oneof_case(hrz_proto::Path&& path)
    {
        const uint32_t case_value = scene_model::get_oneof_case(_model, path);
