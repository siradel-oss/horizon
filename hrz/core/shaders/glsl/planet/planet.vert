...
void main()
{
    ...
    float height = texelFetch(hrz_height_lut, ivec2(gl_VertexID, my_BaseInstance + gl_InstanceID), 0).r;
    ...
    vec3 normal = vec3(normal_xy, sqrt(1.0 - dot(normal_xy, normal_xy)));
    normal = mat3(hrz_frame.view_matrix) * local_normal_rotation * normal;
