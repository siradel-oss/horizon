...
#define CPV(I) (palette.packed_color_stops_value[(I) / 4][(I) % 4])
#define CPE(I) (palette.color_stops_encoded[(I)])
#else // defined(WORKAROUND_004)
// @Workaround(005-Android-LoadDataToStructure)
#define CPV(I) (PALETTE_UBO_PATH.packed_color_stops_value[(I) / 4][(I) % 4])
#define CPE(I) (PALETTE_UBO_PATH.color_stops_encoded[(I)])
...
vec4 PALETTE_FN_NAME(PALETTE_ADDITIONAL_ARGUMENTS float value)
{
    ...
    int num_color_stops = PALETTE_UBO_PATH.num_color_stops;

    if (num_color_stops == 0 || is_nan(value))
    {
        return nan_color;
    }
    ...
    else if (value >= CPV(num_color_stops - 1))
    {
        return decode(color_interpolation_mode, CPE(2 * num_color_stops - 1));
    }

    int low = 0;
    int high = int(num_color_stops - 1);
    int max_iter = int(num_color_stops / 2);

    for (int i = 0; i < HRZ_S_MAX_PALETTE_COLOR_STOPS / 2; ++i)
    {
