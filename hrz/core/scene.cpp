...
struct Scene : public hrz_proto::ICameraService
{
    ...
    {
        hrz_proto::SceneViewIndex index;
        SceneView* view;
        RenderViewInfo render_info;
    };

    std::array<std::optional<SceneViewInstance>, SCENE_VIEW_COUNT> views;
        ...
        override
    {
        if (input.camera_index() >= 0 && (size_t)input.camera_index() < CAMERA_COUNT)
        {
            ...
            lm::dvec2 viewport_pixel_size;
            float aspect_ratio;
            if (lm::is_valid(viewport))
            {
                viewport_pixel_size = lm::dvec2(canvas_size) * lm::size(viewport);
                aspect_ratio = (float)std::abs(viewport_pixel_size.x / viewport_pixel_size.y);
            }
            else
            {
                viewport = lm::bbox2({0, 0}, {0, 0});
                viewport_pixel_size = lm::dvec2(0, 0);
                aspect_ratio = 1.0F;
            }
    ...
    bool has_scene_view(hrz_proto::SceneViewIndex index)
    {
        return hrz_proto::SceneViewIndex_IsValid(index) && views[index].has_value();
    }

    // @Todo(C++26) Return an optional reference instead.
    SceneViewInstance* get_scene_view_or_null(hrz_proto::SceneViewIndex index)
    {
        if (has_scene_view(index))
        {
    ...
    {
        auto index = input.scene_view();
        if (has_scene_view(index))
        {
            auto camera_index = get_current_camera_index(index);
            ViewportInfo viewport_info =
                scene::compute_viewport_info(index, model, device_pixel_ratio, canvas_size);
    ...
    {
        if (auto entry = get_scene_view_or_null(input.scene_view()); entry)
        {
            output = scene::get_view_scale_altitude(entry->view);
        }
        else
        {
            LOG_INVALID_SCENE_VIEW_INDEX(input.scene_view());
    ...
    std::optional<ViewportInfo> compute_viewport_info_for_all_views(hrz_proto::CameraIndex camera)
    {
        ...
        for (const auto& view : views)
        {
            ...
            if (get_current_camera_index(view->index) == camera)
            {
                const auto& info = scene::compute_viewport_info(
                    view->index, model, device_pixel_ratio, canvas_size);
                if (!viewport_info)
                {
                    viewport_info = info;
                }
        ...
        ::hrz_proto::PixelPositionResult& output) override
    {
        ...
        if (auto entry = get_scene_view_or_null(input.scene_view()); entry)
        {
            auto camera_index = get_current_camera_index(entry->index);
            ViewportInfo viewport_info =
                scene::compute_viewport_info(entry->index, model, device_pixel_ratio, canvas_size);
            ...
            if (has_result)
            {
                lm::ibbox2 bbox = scene::get_event_viewport_on_canvas(entry->view);
    ...
    const scene_model::SceneViewSettingsPath& path)
{
    ...
    if (auto entry = scene->get_scene_view_or_null(view_index); entry)
    {
        notify_model_update(entry->view, update_type, path);
    }
}
    ...
    GpuResourceContext* gpu_rc)
{
    ...
    {
        if (scene->views[i].has_value())
        {
            destroy_scene_view(scene->views[i]->view, al, js, &scene->render);
            scene->views[i] = std::nullopt;
        }
    }
    ...
    Scene* scene)
{
    ...
    for (auto& entry : scene->views)
    {
        if (entry.has_value())
        {
            vec.push_back(
                std::make_pair(
                    get_color_output(entry->view),
                    get_color_output_viewport_on_canvas(entry->view)));
        }
    }
    return vec;
}
...
bool handle_event(Scene* scene, const Event& event, float device_pixel_ratio)
{
        ...
        && event.platform.kind == platform::Event::Kind::WindowResized)
    {
        ...
        for (auto& entry : scene->views)
        {
            if (entry.has_value())
            {
                set_canvas_size(entry->view, scene->model, scene->canvas_size, device_pixel_ratio);
            }
        }
    }
    ...
    for (auto& entry : scene->views)
    {
        ...
        ViewportEvent viewport_event = make_viewport_event(entry->view, event);

        if (gizmo_layers::handle_event(
                scene->gizmo_layer_system, viewport_event, entry->index,
                entry->render_info.cam_view_info))
        {
            return true;
        }
        else if (
            editor::handle_event(
                scene->shape_editor, viewport_event, entry->index,
                entry->render_info.cam_view_info))
        {
            return true;
        }

        auto* camera = scene->cameras[scene->get_current_camera_index(entry->index)];
        assert(camera);
        camera->handle_event(viewport_event, entry->index, entry->render_info.cam_view_info);
    }
...
void update_from_model(Scene* scene, AssetsLoader* al, BlobAllocator* ba, JobScheduler* js)
{
    ...
    {
        if (!hrz_proto::SceneViewIndex_IsValid(view_index)) return;

        bool has_view = scene->views[view_index].has_value();
        bool needs_view = (bitset & (1 << view_index)) != 0;

        if (has_view && !needs_view)
        {
            destroy_scene_view(scene->views[view_index]->view, al, js, &scene->render);
            scene->views[view_index] = std::nullopt;
        }
        else if (!has_view && needs_view)
        {
            ...
            scene->views[view_index] = Scene::SceneViewInstance{view_index, view, {}};
    ...
    GpuResourceContext* gpu_rc)
{
    ...
    for (auto& entry : scene->views)
    {
        if (entry.has_value())
        {
            work_start_frame(entry->view, scene->shape_editor, &scene->render);
        }
    }
}
...
{
    if (auto entry = scene->get_scene_view_or_null(scene->main_view); entry)
    {
        return entry->view;
    }
    else
...
static std::string_view notification_payload_str(const hrz_proto::CameraNotification& notification)
{
    ...
    ActorRunner* ar)
{
    ...
    for (const auto& entry : scene->views)
    {
        ...
        auto camera_index = hrz_proto::SceneViewSettingsPathBuilder<hrz::SceneModelAccessor>(
                                scene->model, view.index)
                                .camera()
                                .get();

        if (camera_index >= 0 && (size_t)camera_index < CAMERA_COUNT)
        {
            view_to_camera_index[view.index] = camera_index;
        }

        picking_systems[view.index] = get_picking_system(view.view);

        view_to_viewport_info[view.index] = compute_viewport_info(
            view.index, scene->model, scene->device_pixel_ratio, scene->canvas_size);

        view_to_height_above_terrain[view.index] = get_camera_height(view.view);
    }
    ...
    for (auto& entry : scene->views)
    {
        ...
        view_info.view = view.index;

        const auto& camera_info = scene->cameras[view_to_camera_index[view.index]]->get_info();
        const auto& viewport_info = view_to_viewport_info[view.index];
        ...
        view_info.height_above_terrain = get_camera_height(view.view);
        view_info.perceived_distance =
            camera_info.perceived_distance(view_info.height_above_terrain);

        view.render_info = view_info;
        views_info.push_back(view_info);

        planet_geometries.push_back(get_planet_geometry(view.view));
    }
    ...
    for (auto& entry : scene->views)
    {
        if (entry.has_value())
        {
            scene->render_request |= work(
                entry->view, scene->model, entry->render_info.cam_view_info, scene->canvas_size,
                scene->device_pixel_ratio, cpi, scene->planet, heatmap_repr_registry,
                scene->last_quick_highlight_feature_reference);
        }
    }
    ...
    if (selection::has_changed_since_last_frame(scene->selection) || scene->has_just_added_views)
    {
        ...
        for (auto& entry : scene->views)
        {
            if (entry.has_value())
            {
                set_highlight_enabled(entry->view, enable_highlight);
            }
        }
...
void register_views(Scene* scene)
{
    ...
    for (auto& entry : scene->views)
    {
        ...
        const CameraViewInfo& view_info = view.render_info.cam_view_info;
        my::Renderer::ViewId view_id;

        if (view.index == scene->main_view)
        {
            view_id = my::MainView;
        ...
        register_aux_views(view.view, &scene->render, view_id, scene_view_views);

        view.render_info.view_main = view_id;
        view.render_info.all_views = scene->render.rd->make_view_mask(scene_view_views);
    }
...
void work_gpu(Scene* scene, my::Instance* my, GpuResourceContext* gpu_rc, BlobAllocator* ba)
{
    ...
    for (const auto& entry : scene->views)
    {
        if (entry.has_value())
        {
            views_info.push_back(entry->render_info);
        }
    }
    ...
    for (auto& entry : scene->views)
    {
        if (entry.has_value())
        {
            scene->render_request |= work_gpu(entry->view, &scene->render);
        }
    }
    ...
    if (scene->render_request.is_planet_feedback_scheduled())
    {
        for (auto& entry : scene->views)
        {
            if (entry.has_value())
            {
                planet::request_feedback_render(get_planet_geometry(entry->view));
            }
        }
    }
        ...
        for (auto& entry : scene->views)
        {
            if (entry.has_value())
            {
                vector_flat_overlay::schedule_visual_render(
                    get_vector_flat_overlay(entry->view),
...
void draw(Scene* scene, my::Instance* my, GpuResourceContext* gpu_rc)
{
    ...
    for (auto& entry : scene->views)
    {
        if (entry.has_value())
        {
            ...
            draw(entry->view, scene->render_request, &scene->render, planet_geometry_resources);
        }
    }
...
uint32_t get_oneof_case_model(const Scene* scene, const hrz_proto::Path& path)
{
    return scene_model::get_oneof_case(scene->model, path);
}
    ...
    std::span<const hrz_proto::LayerHandle> included_rasters)
{
    for (auto& entry : scene->views)
    {
        ...
        auto ticket = schedule_pick(entry->view, mouse_position, included_rasters);
        if (ticket.has_value())
        {
            return PositionPickingTicket{
                scene->position_picking_pool.alloc(PositionPicking{entry->index, ticket.value()}),
                entry->index
            };
        }
    }
...
std::optional<AreaPickingTicket> schedule_pick(Scene* scene, lm::ibbox2 rect)
{
    for (auto& entry : scene->views)
    {
        ...
        auto ticket = schedule_pick(entry->view, rect);
        if (ticket.has_value())
        {
            return AreaPickingTicket{
                scene->area_picking_pool.alloc(AreaPicking{entry->index, ticket.value()}),
                entry->index
            };
        }
    }
    ...
    picking::ObjectReference* obj_ref)
{
    ...
    auto view = scene->get_scene_view_or_null(picking_ticket.view);
    if (!view)
    {
    ...
    if (!retrieve_picking_result(view->view, picking_ticket.ticket, &pick_result))
    {
        return false;
    }
    scene->position_picking_pool.release(ticket.ticket);
    ...
    std::vector<hrz_proto::TypedObjectReference>& typed_refs)
{
    ...
    if (!retrieve_picking_result(view->view, picking_ticket.ticket, result))
    {
        return false;
    }
    scene->area_picking_pool.release(ticket.ticket);
...
std::optional<float> fast_query_elevation(Scene* scene, const GeoPosition2& position)
{
    return hrz::planet::fast_query_elevation(scene->planet, position);
}
    ...
    if (mu_begin_window_ex(ctx, window_name, mu_rect(300, 100, 580, 400), MU_OPT_CLOSED))
    {
                ...
                0))
        {
            ...
            for (auto pair : scene->layers_info.layers)
            {
                ...
                // @Safety Taking .data() of the string_view returned by Name() is safe here because
                // the string_view is pointing to a static null-terminated string.
                mu_text(ctx, hrz_proto::LayerType_Name(pair.second.type).data());
...
void viewport_dev_ui(Scene* scene, mu_Context* ctx)
{
    ...
    for (const auto& entry : scene->views)
    {
        ...
        mu_text(
            ctx,
            format_to_buffer(
                buffer, "Scene view {}{}", (int)entry->index,
                entry->index == scene->main_view ? " (main)" : ""));

        // get_event_viewport_on_canvas is used here instead of
        // get_color_output_viewport_on_canvas just to get the more
        // usual top-left-origin coordinates.
        auto bbox = scene::get_event_viewport_on_canvas(entry->view);

        // The bbox has inclusive intervals, so we have to add 1 to
        // get the actual size.
        auto size = lm::size(bbox);
        size += {1, 1};

        mu_text(
            ctx,
            format_to_buffer(
                buffer, "{} x {} ({}, {} -> {}, {})", size.x, size.y, bbox.min.x, bbox.min.y,
...
void camera_dev_ui(Scene* scene, mu_Context* ctx, const char* window_name)
{
    ...
    for (size_t i = 0; i < CAMERA_COUNT; ++i)
    {
        ...
        for (auto& entry : scene->views)
        {
            ...
            scene_views[entry->index] =
                scene->get_current_camera_index(entry->index) == (hrz_proto::CameraIndex)i;
            if (scene_views[entry->index]) view_count += 1;
        }
...
void flat_overlay_dev_ui(Scene* scene, mu_Context* ctx, const char* window_name)
{
    ...
    for (const auto& entry : scene->views)
    {
        ...
        auto scene_view_index = (int)entry->index;
        ...
        if (mu_header_ex(
                ...
                label_buffer.data(), 0))
        {
            vector_flat_overlay::dev_ui(get_vector_flat_overlay(entry->view), ctx);
        }
    }
