...
/**
 * Retrieves the discriminant of a oneof field whose the field pointer to by
 * the path is a part of.
 * Returns 0 if the path does not point to a field of a oneof.
 */
...
/**
 * Retrieves the discriminant of a oneof field from message obj of type In
 * at the given path.
 * Returns 0 if the path does not point to a field of a oneof.
 */
template<typename In, typename CaseType>
CaseType get_oneof_case_part(
    const In& obj,
    std::span<const uint32_t> path)
{
    const uint32_t case_value = get_oneof_case_part_raw(obj, path);
    return (CaseType)case_value;
}
