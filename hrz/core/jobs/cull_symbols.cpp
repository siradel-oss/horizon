...
// Horizon decluttering
//
// When looking at the horizon many tiles and many symbols can be visible. This can make the
// scene crowded, and combined with overlap culling, make symbol culling unstable and seemingly
// arbitrary. To avoid this we compute how much each anchor is at a problematic position, i.e.
// how much other anchors in the surroundings can appear close to one another on screen. The
// anchors whose positions lead to crowding are culled.
//
// For each anchor we measure the rate at which, in relative terms, the distance to it changes as
// it moves up the screen, staying on its own horizontal plane, per logical pixel of screen height:
//
//     relative_ground_step = pixel_size_in_meters / tan(inclination)
//
// where `inclination` is the angle between the line of sight and the horizontal plane *at the
// anchor*, obtained from the anchor's own ENU up axis.
//
// Derivation:
//
// Let the anchor sit on its own horizontal plane H, let the camera overlook H from a height Δh,
// let d be the distance from the camera to the anchor, and let θ be the inclination angle, so
// that Δh, d and θ form a right triangle:
//
//     sin(θ) = Δh / d          hence          d(θ) = Δh / sin(θ)
//
// Sliding the anchor within H, directly away from the camera, is what makes it climb towards the
// horizon on screen. The camera does not move and H does not move, so Δh stays constant and d
// depends on θ alone. It can be differentiated like so:
//
//     d'(θ) = -Δh * cos(θ) / sin(θ)^2
//
// When a is a small value, the differential gives:
//
//     d(θ + a) ≈ d(θ) + d'(θ) * a
//
// So the difference ratio is:
//
//     d(θ + a) / d(θ) ≈ (d(θ) + d'(θ) * a) / d(θ)
//
// And the relative difference (0: no change, and 1: 100% change) is:
//
//     (d(θ + a) - d(θ)) / d(θ) ≈ d'(θ) * a / d(θ)
//
//     d'(θ) * a / d(θ) = (-Δh * cos(θ) / sin(θ)^2) * a / (Δh / sin(θ))
//                      = (-Δh * cos(θ) / sin(θ)^2) * sin(θ) * a / Δh
//                      = (-Δh * cos(θ) / sin(θ)) * a / Δh
//                      = (cos(θ) / sin(θ)) * a
//                      = a / (sin(θ) / cos(θ))
//                      = a / tan(θ)
//
// Rotating the anchor by a small angle a therefore changes its distance by about |a / tan(θ)|.
//
// One logical pixel of screen height is pixel_size_in_meters. Because the angle made by a
// one-pixel displacement is small, the value can be interpreted as an angle in radians without
// introducing a large error. (It's a vertical length at a distance of one metre.) Substituting
// it for a gives the first formula.
// Note that Δh cancelled out, this makes the values independent of camera height.
//
// This is computed for every anchor and the value is compared to a set threshold value. All
// anchors with a value above the threshold are culled.
...
struct TransformedAnchor
{
    ...
    // Relative change in the distance to the anchor caused by moving it up by one logical pixel
    // of screen height, along its own horizontal plane. Infinite when the line of sight grazes
    // that plane.
    float relative_ground_step;
};
    ...
    const hrz::vt::AnchorCullingInfo& anchor)
{
    ...
    {
        // See the note at the top of this file regarding horizon de-cluttering.
        //
        // sin(θ) = Δh / d
        //     Δh = sin(θ) * d
        // Δh is obtained by the dot product.
        //
        // If g is the distance on the anchor's horizontal plane between the anchor and
        // the projection of the camera's position on that plane:
        // cos(θ) = g / d
        //      g = cos(θ) * d
        // g is obtained by the norm of the cross product.
        //
        // Dividing the second value by the first gives tan(θ).
        float distance_times_sin = std::abs(lm::dot(anchor_pos_cc, anchor.local_up_axis));
        float distance_times_cos = lm::length(lm::cross(anchor_pos_cc, anchor.local_up_axis));
        out.relative_ground_step = distance_times_sin > 0.0F
            ? view.pixel_size_in_meters * distance_times_cos / distance_times_sin
        ...
        for (const auto& span : anchor_spans)
        {
            ...
            for (uint32_t i = 0; i < span.anchor_count; ++i)
            {
                ...
                if (transformed.depth < 1 && lm::intersect(screen_bbox, transformed.screen_bbox)
                    && (max_relative_ground_step <= 0.0F
                        || transformed.relative_ground_step <= max_relative_ground_step)
    ...
    const JobContext& context)
{
    ...
    {
        if (!cull_one_view(params.groups, view, params.max_relative_ground_step, response, context))
        {
