    ...
    hrz::flat_hash_set<SignedTileCoords>& selected_tiles)
{
    ...
    uint64_t tile_count_at_lod = (uint64_t)1 << params.tile_coords.lod;
    ...
    hrz_jobs::ReprojectedTiles& response)
{
    ...
    if (raster_geometry.tiling_scheme.scheme_type_case()
        == hrz_proto::TilingSchemeParams::SchemeTypeCase::kGlobalTiling)
    {
        assert(raster_geometry.tiling_scheme.has_global_tiling());
        ...
        raster_geometry.tiling_scheme.scheme_type_case()
        == hrz_proto::TilingSchemeParams::SchemeTypeCase::kLocalTiling)
    {
        ...
        raster_geometry.tiling_scheme.scheme_type_case()
        == hrz_proto::TilingSchemeParams::SchemeTypeCase::SCHEME_TYPE_NOT_SET)
    {
        // Deprecated, no-op
    }
    else
    ...
    const JobContext&)
{
    ...
    const auto tiling_scheme_type = raster_geometry.tiling_scheme.scheme_type_case();
    if (tiling_scheme_type == hrz_proto::TilingSchemeParams::SchemeTypeCase::kGlobalTiling
