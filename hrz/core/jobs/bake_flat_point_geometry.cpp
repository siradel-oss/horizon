...
void generate_points_geometry(
    ...
    const lm::ubvec4& disc_color_rgba,
    float disc_radius,
    const lm::ubvec4& outline_color_rgba,
    float outline_width)
{
    auto append_vertex = [&](const lm::dvec3& position)
    {
        ...
        point_data.push_back(
            {{}, disc_color_rgba, disc_radius, outline_color_rgba, outline_width, feature_index});
    };
    ...
    const JobContext& context)
{
    ...
    for (const auto& instance : style.instances.get_data())
    {
        ...
        lm::ubvec4 outline_color_srgb = input.default_outline_color_srgb;
        float outline_width = input.default_outline_width;
        ...
        for (uint64_t j = prp_begin; j < prp_end; ++j)
        {
            ...
            else if (style_prps[j] == input.outline_color_prp)
            {
                outline_color_srgb = style_values.as_color(j);
            }
            else if (style_prps[j] == input.outline_width_prp)
            {
                outline_width = (float)style_values.as_number(j);
            }
        }
        ...
        generate_points_geometry(
            ...
            disc_radius, outline_color_srgb, outline_width);
    }
