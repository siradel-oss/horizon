    ...
    bool prioritize_transparent_images)
{
    struct ImageFormat
    {
        ...
        bool supports_tranparency;
    };

    // Ordered by decreasing order of preference, mainly based on
    // file weight.
    // "image/jpg" is technically incorrect, but some servers use it
    // nonetheless.
    // "image/jpgpng" is even less correct, but is common among WMTS
    // servers. It usually returns PNGs, but maybe not always. This
    // is why it is present twice in the list. The second one stands
    // for "we haven't been able to satisfy our requirements with
    // proper MIME types, so maybe this will give us what we want".
    // "image/vnd.jpeg-png" and "image/vnd.jpeg-png8" are proprietary
    // MIME types from GeoServer, combining JPEG and PNGs tiles,
    // using the latter when an alpha channel is needed. See
    // https://www.geosolutionsgroup.com/blog/geoserver-smart-transparency/
    // "image/png32", "image/png24", and "image/png8" are non-standard
    // MIME types, used at least by GeoServer. See
    // https://docs.geoserver.org/stable/en/user/services/wms/outputformats.html
    constexpr ImageFormat formats[] = {
        {"image/webp", true},           {"image/jpgpng", true},       {"image/jpeg", false},
        {"image/jpg", false},           {"image/vnd.jpeg-png", true}, {"image/png24", false},
        {"image/png32", true},          {"image/png", true},          {"image/vnd.jpeg-png8", true},
        {"image/png; mode=8bit", true}, {"image/png8", true},         {"image/gif", true},
    };

    std::optional<std::string_view> best_format = std::nullopt;
    bool best_format_supports_transparency = false;
    size_t best_format_score = HRZ_ARRAY_COUNT(formats);

    for (const auto& format_node : get_map_node.children("Format"))
    {
        ...
        for (size_t i = 0; i < HRZ_ARRAY_COUNT(formats); ++i)
        {
            ...
            if (mime_type == format.mime_type)
            {
                size_t score = i;
                bool score_is_better = score < best_format_score;

                bool is_better = [&]() -> bool
                {
                    if (prioritize_transparent_images)
                    {
                        if (format.supports_tranparency)
                        {
                            if (!best_format_supports_transparency)
                            {
                                return true;
                            }
                            else
                            {
                                return score_is_better;
                            }
                        }
                        else
                        {
                            return score_is_better && !best_format_supports_transparency;
                        }
                    }
                    return score_is_better;
                }();
...
//     WHERE auth_name = "epsg" AND has_flipped_axes = true;
// (The auth_name filter is to make sure authorities aren't mixed, but there
// are only EPSG results anyway.)
...
static constexpr uint16_t kFlippedEpsgSrids[] = {
    ...
    2130,  2131,  2132,  2166,  2167,  2168,  2169,  2170,  2171,  2172,  2173,  2174,  2175,
    2176,  2177,  2178,  2179,  2180,  2193,  2199,  2200,  2206,  2207,  2208,  2209,  2210,
    2211,  2212,  2319,  2320,  2321,  2322,  2323,  2324,  2325,  2326,  2327,  2328,  2329,
    ...
    2538,  2539,  2540,  2541,  2542,  2543,  2544,  2545,  2546,  2547,  2548,  2549,  2551,
    ...
    2734,  2735,  2738,  2739,  2740,  2741,  2742,  2743,  2744,  2745,  2746,  2747,  2748,
    2749,  2750,  2751,  2752,  2753,  2754,  2755,  2756,  2757,  2758,  2935,  2936,  2937,
    2938,  2939,  2940,  2941,  2953,  3006,  3007,  3008,  3009,  3010,  3011,  3012,  3013,
    3014,  3015,  3016,  3017,  3018,  3019,  3020,  3021,  3022,  3023,  3024,  3025,  3026,
    3027,  3028,  3029,  3030,  3034,  3035,  3038,  3039,  3040,  3041,  3042,  3043,  3044,
    3045,  3046,  3047,  3048,  3049,  3050,  3051,  3058,  3059,  3068,  3114,  3115,  3116,
    3117,  3118,  3120,  3126,  3127,  3128,  3129,  3130,  3131,  3132,  3133,  3134,  3135,
