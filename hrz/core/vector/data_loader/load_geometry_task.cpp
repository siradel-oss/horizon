...
{
    uint64_t hash = hrz::index_of_variant<decltype(Task::data), Task::LoadGeometry>();
...
template<>
void VectorDataLoader::clear_task<VectorDataLoader::Task::LoadGeometry>(
    Task& task,
    Task::LoadGeometry& task_data,
    JobScheduler* js)
{
...
template<>
void VectorDataLoader::cancel_task_jobs<VectorDataLoader::Task::LoadGeometry>(
    Task& task,
    Task::LoadGeometry& task_data,
    JobScheduler* js)
{
...
template<>
void VectorDataLoader::unload_task_data<VectorDataLoader::Task::LoadGeometry>(
    Task& task,
    Task::LoadGeometry& task_data,
...
template<>
void VectorDataLoader::work_new_task<VectorDataLoader::Task::LoadGeometry>(
    ...
    if (layer_model.geometry_source != LayerModel::NO_SOURCE)
    {
        task_data.load_vector_data_task = TaskDependency::between_tasks(
            get_or_create_load_vector_data_task(
                task_data.layer_model, layer_model.geometry_source, task_data.feature_selection),
            task_ref);
...
template<>
void VectorDataLoader::work_unloaded_task<VectorDataLoader::Task::LoadGeometry>(
    WeakTaskRef& task_ref,
    Task& task,
    Task::LoadGeometry& task_data)
{
...
template<>
void VectorDataLoader::work_loading_task<VectorDataLoader::Task::LoadGeometry>(
    WeakTaskRef& task_ref,
    Task& task,
    Task::LoadGeometry& task_data,
    JobScheduler* js,
...
template<>
bool VectorDataLoader::unload_task_data_if_not_needed<VectorDataLoader::Task::LoadGeometry>()
{
    return true;
}

template<>
void VectorDataLoader::check_for_invalidated_data_for_task<VectorDataLoader::Task::LoadGeometry>(
    WeakTaskRef& task_ref,
    Task& task,
    Task::LoadGeometry& task_data,
    JobScheduler* js)
