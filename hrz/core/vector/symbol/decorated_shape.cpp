    ...
    const std::function<uint32_t(const hrz_proto::SymbolElement&)>&)
{
    ...
    {
        case hrz_proto::DECORATED_SHAPE_CIRCLE:
        case hrz_proto::DECORATED_SHAPE_RECTANGLE:
        case hrz_proto::DECORATED_SHAPE_CAPSULE: break;
        case hrz_proto::DECORATED_SHAPE_REGULAR_POLYGON:
    ...
    Render* render)
{
    if (prototype_handle.type != ElementType || baked_instances.type != ElementType)
    {
        ...
        HRZ_LOG_ERROR(
            "Unexpected element type: expect {}, got {}", (int)ElementType,
            (int)baked_instances.type);
        return std::nullopt;
    }
