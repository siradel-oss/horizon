...
struct SymbolCullingSystem
{
    ...
    float last_cull_max_relative_ground_step = -1.0F;
    ...
    std::span<const RenderViewInfo> views_info)
{
    ...
    float max_relative_ground_step =
        0.01F * hrz::get_value(hrz::Value::DebugSymbolHorizonDeclutteringPercent);

    if (!sys->cull_ticket
        && (sys->last_cull_start_time_s + CullDelayS < hrz::clock::CurrentFrameWallTime.s)
        && (sys->should_update()
            || max_relative_ground_step != sys->last_cull_max_relative_ground_step))
    {
        ...
        params.max_relative_ground_step = max_relative_ground_step;
        sys->last_cull_max_relative_ground_step = max_relative_ground_step;
        ...
        sys->last_cull_start_time_s = hrz::clock::CurrentFrameWallTime.s;
    }
