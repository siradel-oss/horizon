...
void ReprRegistry::register_repr(
    hrz_proto::VectorRepr::ReprTypeCase type,
    std::unique_ptr<ReprSystem> repr)
{
...
ReprSystem& ReprRegistry::get(hrz_proto::VectorRepr::ReprTypeCase type)
{
    ...
    {
        HRZ_LOG_WARNING("Fetching unknown representation type {}", (int)type);
        assert(!"Representation system has not been registered yet");
        return *_reprs[hrz_proto::VectorRepr::ReprTypeCase::REPR_TYPE_NOT_SET];
    }
}
