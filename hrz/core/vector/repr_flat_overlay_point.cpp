...
public:
            ...
            register_prp) override
    {
        ...
        const std::string_view disc_radius_prp_name = repr.flat_overlay_point().radius().name();
        ...
        config->color_prp = register_prp(
            ...
            hrz::vector_data::attr_from_color<hrz::vector_data::OwnedAttributeValue>(
                config->default_color_srgb));
        config->radius_prp = register_prp(disc_radius_prp_name, config->default_radius);

        config->z_index = repr.flat_overlay_point().z_index();
        config->clip_to_tile = repr.flat_overlay_point().clip_to_tile();

        config->outline_color = hrz::srgb_to_linear(
            hrz::convert_proto_color_to_float(repr.flat_overlay_point().outline_color()));
        config->outline_width = repr.flat_overlay_point().outline_width();
    ...
    void initialize_tile_with_config(const Config& config, Tile* tile) override
    {
        ...
        tile->ubo_template.disc_outline_width = config.outline_width;
        tile->ubo_template.disc_outline_color = config.outline_color;
        tile->ubo_template.disc_radius_unit = config.radius_unit;
