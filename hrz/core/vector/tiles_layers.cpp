...
#include "hrz/core/scene_path/vector/style_paths.h"
...
VectorTilesLayerSystem* create_system(PickingIdAllocator* picking_id_allocator)
{
    ...
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::REPR_TYPE_NOT_SET, vt::create_null_repr_system());
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::kExtrudedGeometry, vt::create_extruded_repr_system());
    system->reprs.register_repr(
    ...
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::kFlatOverlayPoint,
        vt::create_flat_overlay_point_repr_system());
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::kModel, vt::create_model_repr_system());
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::kCylinder, vt::create_cylinder_repr_system());
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::kHeatmap, vt::create_heatmap_repr_system());
    system->reprs.register_repr(
        hrz_proto::VectorRepr::ReprTypeCase::kSymbol, vt::create_symbol_repr_system());
    ...
    const char* window_name)
{
    ...
    if (mu_begin_window_ex(ctx, window_name, mu_rect(300, 200, 400, 370), MU_OPT_CLOSED))
    {
        ...
        if (mu_header(ctx, "Layers"))
        {
            if (!system->global_layer_id_to_handle.empty())
            {
                for (auto pair : system->global_layer_id_to_handle)
                {
                    ...
                    if (mu_begin_treenode_ex(
                            ctx, hrz::format_to_buffer(buffer, "{}", pair.first), layer_name, 0))
                    {
        ...
        {
            {
                static int layout[2] = {180, -1};
                mu_layout_row(ctx, 2, layout, 0);
                mu_label(ctx, "Symbol horizon de-cluttering (%)");
                mu_slider_ex(
                    ctx, get_value_ptr(Value::DebugSymbolHorizonDeclutteringPercent),
                    get_value_min(Value::DebugSymbolHorizonDeclutteringPercent),
                    get_value_max(Value::DebugSymbolHorizonDeclutteringPercent), 0.05F, "%.2f",
                    MU_OPT_ALIGNCENTER);
            }
        }
