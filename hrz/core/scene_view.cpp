...
// Scissors are clamped to pixel boundaries to make views more stable.
lm::bbox2 clamp_to_pixels(lm::bbox2 bbox, const lm::uvec2& canvas_size)
{
    auto clamp_to_pixel = [](float v, uint32_t size) { return std::round(v * size) / (float)size; };

    bbox.min.x = clamp_to_pixel(bbox.min.x, canvas_size.x);
    bbox.min.y = clamp_to_pixel(bbox.min.y, canvas_size.y);
    bbox.max.x = clamp_to_pixel(bbox.max.x, canvas_size.x);
    bbox.max.y = clamp_to_pixel(bbox.max.y, canvas_size.y);

    return bbox;
}

lm::bbox2 sanitize(lm::bbox2 bbox)
{
    if (lm::is_valid(bbox))
    {
        return bbox;
    ...
    return lm::bbox2({0.0F, 0.0F}, {0.0F, 0.0F});
}
    ...
    lm::uvec2 canvas_size)
{
    ...
    lm::bbox2 viewport = sanitize(clamp_to_pixels(
        invert_y_axis(to_lm(viewport_settings.viewport()), {1.0F, 1.0F}), canvas_size));
    lm::bbox2 subfrustum = sanitize(clamp_to_pixels(
        invert_y_axis(to_lm(viewport_settings.scissor()), {1.0F, 1.0F}), canvas_size));
    ...
    picking::FeatureReference quick_highlight_feature_id)
{
    ...
    if (view->terrain_settings_updated)
    {
        ...
        view->dtm_enabled = !terrain_settings.disable_terrain_elevation();
    ...
    frame_uniforms_data.dtm_enabled = view->dtm_enabled;
    ...
    frame_uniforms_data.time = hrz::clock::CurrentFrameSimTime.s;
    ...
    const planet::GeometryResources& planet_geometry_resources)
{
    ...
    auto viewport_size = lm::size(view->viewport_on_canvas);
