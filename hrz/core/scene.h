...
/**
 * Returns the case of a oneof field in the scene model.
 */
uint32_t get_oneof_case_model(const Scene*, const hrz_proto::Path&);
...
/**
 * Returns elevation at the provided position. The value is
 * computed from what is already available in the DTM raster
 * collection. No new data is fetched.
 */
std::optional<float> fast_query_elevation(Scene*, const GeoPosition2& position);
