...
def generate_lookup_function(node, parent_prefix = ""):
    code_before = ""
    code = f"std::optional<uint32_t> html_color_lookup{"_" + parent_prefix if len(parent_prefix) > 0 else ""}(std::string_view str)\n"
    code += "{\n"
    if "" in node:
    ...
    for prefix, child in node.items():
        ...
        if isinstance(child, dict):
            ...
            code += f"            if (hrz::str::starts_with(str, \"{prefix}\")) {{ return html_color_lookup_{full_prefix}(str.substr({len(prefix)})); }}\n"
...
def tests_cmd():
    ...
    print(f"    static const char* color_names[] = {{{ ", ".join([f'"{randomize_case(c[0])}"' for c in COLORS])}}};")
    print(f"")
    print(f"    static const uint32_t expected_values[] = {{{ ", ".join([c[1] for c in COLORS]) }}};")
    print(f"")
