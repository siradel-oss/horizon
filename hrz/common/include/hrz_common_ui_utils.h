...
// content.
static void begin_sticky_panel(mu_Context* ctx, StickyPanelState* state, const char* name)
{
    mu_begin_panel(ctx, name);
    state->panel = mu_get_current_container(ctx);
}

static void end_sticky_panel(mu_Context* ctx, StickyPanelState* state)
{
    mu_end_panel(ctx);
...
static void add_tooltip(mu_Context* ctx, TooltipContext* tooltip_ctx, std::string_view text)
{
...
static void draw_tooltips(mu_Context* ctx, TooltipContext* tooltip_ctx)
{
...
static int begin_layout_treenode(
    mu_Context* ctx,
    void* id_data,
    size_t id_size,
...
static void end_layout_treenode_header(mu_Context* ctx, bool expanded)
{
...
static void end_layout_treenode(mu_Context* ctx, bool expanded)
{
    if (expanded)
    {
        mu_layout_end_column(ctx);
