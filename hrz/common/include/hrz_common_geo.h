...
static lm::dvec3 web_mercator_to_ecef(lm::dvec2 web_mercator, double altitude = 0)
{
...
static GeoPosition2 normalize(const GeoPosition2& position)
{
    return {clamp_latitude(position.lat), normalize_longitude(position.lon)};
}

static GeoPosition3 normalize(const GeoPosition3& position)
{
...
static GeoVolumeBounds normalize(const GeoVolumeBounds& bounds)
{
...
static double distance_to_wgs84_bbox(const lm::dvec3& ecef_pos, const GeoVolumeBounds& wgs84_bounds)
{
