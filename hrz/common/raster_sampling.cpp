    ...
    hrz_proto::ImageFormat image_format)
{
    ...
    if (nodata_handling == hrz_proto::NodataHandling::IGNORE_NODATA
        || nodata_value.value_type_case() == hrz_proto::NodataValue::VALUE_TYPE_NOT_SET)
    {
        ignore_nodata();
    ...
    auto make_rgb_scalar_nodata_function = [&](uint32_t (*encode_func)(float))
    {
        ...
        switch (nodata_value.value_type_case())
        {
            case hrz_proto::NodataValue::kBitPattern:
            {
            ...
            case hrz_proto::NodataValue::kUintValue:
            {
                nodata_pattern = encode_func((float)nodata_value.uint_value());
                break;
            }
            case hrz_proto::NodataValue::kIntValue:
            {
                nodata_pattern = encode_func((float)nodata_value.int_value());
                break;
            }
            case hrz_proto::NodataValue::kFloatValue:
            {
                nodata_pattern = encode_func(nodata_value.float_value());
                break;
            }
            case hrz_proto::NodataValue::kFloatNan:
            case hrz_proto::NodataValue::kColor:
            {
                ignore_nodata();
                break;
            }
    ...
    switch (image_format)
    {
            ...
            switch (nodata_value.value_type_case())
            {
                ...
                case hrz_proto::NodataValue::kColor:
                {
                    ...
                    const lm::ubvec4 rgba = {
                        (uint8_t)std::round(color.r() * 255), (uint8_t)std::round(color.g() * 255),
                        (uint8_t)std::round(color.b() * 255), (uint8_t)std::round(color.a() * 255)
                    };
        ...
        case hrz_proto::ImageFormat::SIRADEL_LEGACY_F32:
        {
            switch (nodata_value.value_type_case())
            {
                ...
                {
                    nodata_pattern =
                        hrz::encode_float_to_siradel_legacy_f32((float)nodata_value.uint_value());
                    break;
                }
                case hrz_proto::NodataValue::kIntValue:
                {
                    nodata_pattern =
                        hrz::encode_float_to_siradel_legacy_f32((float)nodata_value.int_value());
                    break;
                }
                case hrz_proto::NodataValue::kFloatValue:
                {
