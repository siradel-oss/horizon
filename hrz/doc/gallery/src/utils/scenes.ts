...
async function applySceneDump(api: HrzApi.AsyncApi, url: string, clearLayers: boolean) {
    let resp = await fetch(url);
    if (resp.ok) {
        await api.SceneDumpService.loadDump({
            clearLayers,
            dump: HrzProtocol.SceneDump.decode(new Uint8Array(await resp.arrayBuffer())),
        });
    }
}

export async function applySceneTemplate(api: HrzApi.AsyncApi, name: string) {
    let url = `scene/template/${name}.hrz_scene.pbf`;
    await applySceneDump(api, url, false);
}

export async function applyScene(api: HrzApi.AsyncApi, name: string) {
    let url = `scene/${name}.hrz_scene.pbf`;
    await applySceneDump(api, url, true);
}

export async function applyDefaultOrthoBaseLayer(api: HrzApi.AsyncApi) {
...
export async function applyDefaultSymbolicBaseLayer(api: HrzApi.AsyncApi) {
