...
let dataLayer = ref<HrzProtocol.ILayerHandle | null>(null);
let tileLayer = ref<HrzProtocol.ILayerHandle | null>(null);
...
async function onHorizonReady(api_: HrzApi.AsyncApi) {
    ...
    await applyScene(api.value, "map_localization").then(async function () {
        dataLayer.value = (await getLayerByName(api.value!, "HRZ Basemap places data")) || {};
        tileLayer.value = (await getLayerByName(api.value!, "HRZ Basemap places")) || {};
    });
}
