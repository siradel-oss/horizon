...
async function respondToVectorDataRequest(msg: HrzProtocol.IVectorDataRequestMessage) {
    ...
    for (const featureId of msg.featureIdSelection?.featureIds || []) {
        ...
        const feature = new HrzProtocol.ClientFeature();
...
watch(selectedSetup, async (newSetup, oldSetup) => {
    if (api && vectorTilesLayer && newSetup !== undefined) {
        ...
        if (legendValues.value && legendColors.value && palette.colorPoints) {
            legendValues.value.innerHTML = "";

            const colorPointCount = palette.colorPoints.length || 0;

            for (let i = colorPointCount - 1; i >= 0; i--) {
                const value = palette.colorPoints[i].value || 0;
...
function displayPickResult(msg: HrzProtocol.IPickResults) {
...
<template>
    <SplitView>
        ...
        <template #right>
            ...
            <div class="absolute top-4 right-4 bottom-4 flex flex-col justify-center">
                <div
