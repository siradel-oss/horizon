...
async function onHorizonReady(api: HrzApi.AsyncApi, msgHandler: MessageHandler) {
    ...
    const params: HrzProtocol.IMapboxTranslationParams = {
        styleJson: scene,
    ...
    msgHandler.watch((msg) => {
        if (
            msg.type != HrzProtocol.MessageType.MAPBOX_TRANSLATION_MESSAGE ||
            !msg.mapboxTranslation
        ) {
            return false;
        }

        if (msg.mapboxTranslation.ticket?.opaque != ticket.opaque) {
            return false;
        }
    ...
    <SplitView>
        <template #left>
            <div class="typography-normal">
                ...
                <p>
                    Please refer to <a href="../mapbox_scenes.html">the documentation</a> for
