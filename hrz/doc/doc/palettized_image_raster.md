...
[Palettized image rasters](HrzProtocol.PalettizedImageRasterProviderParams.html) are displayed on the planet according to their internal provider and [numeric palette](numeric_palettes.html). The internal provider retrieves image pixels values as numbers then applies the given palette to colorize these values. The internal provider’s [image format](raster_image_formats.html) must be one that contains scalar values (and not colours).

The internal provider can define a nodata value. The `nodata_color` property of [[PalettizedImageRasterProviderParams]] enables mapping pixels with this nodata value to a given color without having to know the specific value in advance.
