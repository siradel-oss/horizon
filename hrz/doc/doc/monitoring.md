...
Profiling features can be accessed using either through the development UI (when it has been enabled in the [viewer options](reference/HrzProtocol.ViewerOptions)) under `Monitoring` > `Remote monitoring`, or by using the methods of [the monitoring service](reference/HrzProtocol.MonitoringService) from the API.
...
Horizon can be asked to send the monitoring information through the message queue with the [`EnableMonitoringMessages` method of `MonitoringService`](reference/HrzProtocol.MonitoringService.html#method-EnableMonitoringMessages). When that option is enabled, messages with a [`MonitoringDataMessage`](reference/HrzProtocol.TypedMessage) payload will be regularly sent to the client.

The format is described in [MonitoringDataMessage]($proto). The WebSocket interface uses the same format.

The `HrzMonitoringProtocol.MonitoringMessage` type is defined separately from Horizon's main protocol definitions, and its `.proto` file is available in the source tree as `//hrz/monitoring/proto:hrz_monitoring_interface.proto`. TypeScript bindings are available through the `@siradel-oss/horizon-monitoring-protocol` npm package.
