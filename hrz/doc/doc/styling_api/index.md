...
* For a [Vec2fProperty]($proto) the name is appended with `_x`, `_y`.
* For a [Vec3fProperty]($proto) the name is appended with `_x`, `_y`, `_z`.
...
| `\n`            | Line break |
| `\t`            | Tabulation |
| `\u{hhhh}`      | Unicode code point, hexadecimal. For example `\u{e9}` is `é`, `\u{6587}` is `文`.  |
...
Each features in each evaluation context has its own random number generator (RNG) state. The RNG is seeded with the feature ID to ensure reproducibility across tiles of different zoom levels. This means that a feature in a dataset will always be stylized the same way for a given styling script.
...
| `rand_norm_u(mean, std: number) -> number` | Random unsigned unsigned integer number following a normal distribution |
...
| `to_int(expr: any) -> int` | Converts a value to an integer using the corresponding [attribute transform](reference/HrzProtocol.AttributeTransform). |
| `to_uint(expr: any) -> uint` | Converts a value to an unsigned integer using the corresponding [attribute transform](reference/HrzProtocol.AttributeTransform). |
| `to_number(expr: any) -> number` | Converts a value to a real number using the corresponding [attribute transform](reference/HrzProtocol.AttributeTransform). |
| `to_string(expr: any) -> string` | Converts a value to a string using the corresponding [attribute transform](reference/HrzProtocol.AttributeTransform). |
| `to_boolean(expr: any) -> boolean` | Converts a value to a boolean using the corresponding [attribute transform](reference/HrzProtocol.AttributeTransform). |
| `to_color(expr: any) -> color` | Converts a value to a color using the corresponding [attribute transform](reference/HrzProtocol.AttributeTransform). |
