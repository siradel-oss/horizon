...
> When the amount of video memory is low, the engine can decide to use less than three groups internally. You can still use the three groups in the layers definitions, and they will still be ordered accordingly, however the optimisations mentioned above might not be available. This can be overridden with the `imagery_merge_group_count` field of [GraphicsSettingsOverrides]($proto) during engine initialisation.
>
...
DTM rasters are all merged in a single group and thus the [DTM layer](reference/HrzProtocol.DtmRasterLayer) doesn't provide a setting for the group.
