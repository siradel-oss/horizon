...
Additionally the actors can be run from the main thread instead of a dedicated one, through the `run_actors_on_main_thread` property of [ViewerOptions]($proto). It is not recommended to use this option in typical production contexts as it is targeted towards debugging usage.
