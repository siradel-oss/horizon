...
Each root type is associated with a root protocol message type, which is itself the description of the tree structure. For example, a scene view [root](reference/HrzProtocol.PathRoot) uses a [SceneViewIndex]($proto) as parameter, and points to a [SceneViewSettings]($proto) structure.

## Layers

Layers are a type of root nodes that essentially declare how and what is going to be displayed in the scene. A layer is only described by its [type](reference/HrzProtocol.LayerType), and identified by its [handle](reference/HrzProtocol.LayerHandle). They are created, destroyed, and queried by using the [Layer service](reference/HrzProtocol.LayerService).
...
The scene model is provided with a root node, called [SceneViewSettings]($proto), that is used to configure general aspects of a scene view. It uses the scene view index as parameter. This is shown in the example below (read the following sections for more details on the use of paths).
...
The scene settings can be configured through the [SceneSettings]($proto) root. This node can be seen as general options configuration therefore it does not take any parameters. Those settings configure what views are in use.
...
Similarly to scene view settings, there is a scene model root for [CameraSettings]($proto), that is used to configure view-independent aspects of the camera, for each camera.
...
The user is expected to use the scene model API, which allows read and write access to the scene model in a type-safe manner, instead interacting with the [SceneModelService]($proto) directly. If you need to use the [SceneModelService]($proto), for example to develop tooling, see its documentation for how it works, and how paths are encoded.

At the heart of this API are paths. Paths describe a location in the scene model. They all start from a [root](reference/HrzProtocol.PathRoot).
