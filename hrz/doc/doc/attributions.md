...
In order to enable attributions feedback, call the `SetAttributionEnabled` method of [[ViewerService]]. When enabled, the engine will periodically send an [[AttributionsMessage]] listing the attributions to display through the [message queue](message_queue.html).

## Displaying attributions

Each [[Attribution]] may contain some text (which itself might be HTML), and a logo URL. All those elements must be displayed. Please refer to your data providers to check for guidelines.
