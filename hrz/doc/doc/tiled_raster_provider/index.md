...
In Horizon, such rasters can be displayed using a [tiled raster provider](reference/HrzProtocol.TiledRasterProviderParams).
...
If the image does not fill the whole tiling space, the `bounds` property of [RasterGeometry]($proto) can be used to specify the covered area.
