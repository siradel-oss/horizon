...
For C++, there is only one interface, named `Backend`. However there are two interfaces in TypeScript: `HrzApi.SyncBackend` and `HrzApi.AsyncBackend`(in the `@siradel-oss/horizon-api` package).
