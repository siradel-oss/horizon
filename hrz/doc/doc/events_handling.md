...
This is done with the `SendEvents` API method available in the [[ViewerService]].

```ts
let stream: HrzProtocol.IEventsStream;
...
!!! note ""
    When events are managed outside Horizon, the engine may still process and capture events internally. To completely shutdown internal events management, Horizon provides the `disable_events_capture` options in the [[ViewerOptions]].
...
To this extent Horizon provides a list of actions that can be bound to keys. The bindings are specified in the viewer options at startup (see the example below). See [[KeyAction]] for the full list of key-bindable actions.
