...
Heightmap tiles can be retrieved by any raster provider that supports specifying the image format, such as the [tiled image provider](tiled_raster_provider.html), or the [single image provider](raster_providers.html#untiled-image).
...
Contrarily to imagery raster layers, DTM raster layers cannot have they visibility configured per view. All DTM layers are always used combined together.

It is however possible to opt out of DTM raster layers entirely for a given scene view, in the [terrain settings](terrain_settings.html#terrain-settings-1).
