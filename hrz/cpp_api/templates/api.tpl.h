...
public:
    ...
    std::string get_raw(const hrz_proto::Path& path)
    {
        ...
        return std::string{response.value()};
    }
    ...
    template<typename T>
    T get_oneof_case(const hrz_proto::Path& path)
    {
        hrz_proto::UInt32Value case_value;
        _api->scene_model_service.get_oneof_case(path, case_value);
        return (T)case_value.value();
    }
