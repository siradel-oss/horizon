...
bool migration_5c6d28db_to_37dc6665(const DynamicMessage& src, DynamicMessage* dst)
{
    auto migrate_layer_fn = [&](const DynamicMessage& src, DynamicMessage* dst) -> bool
    {
        auto dst_style = dst->get_message("style");
        ...
        for (int j = 0; j < repr_count; ++j)
        {
            ...
            if (src_repr.get_enum("type") == "FLAT_OVERLAY_POINT_VECTOR_REPR")
            {
                auto dst = dst_repr.get_message("flat_overlay_point");
                auto src = src_repr.get_message("flat_overlay_point");

                auto dst_outline_color = dst.create_message("HrzProtocol.ColorProperty");
                dst_outline_color.copy_message("default_value", src.get_message("outline_color"));
                dst.set_message("outline_color", dst_outline_color);

                auto dst_outline_wodth = dst.create_message("HrzProtocol.FloatProperty");
                dst_outline_wodth.set_float("default_value", src.get_float("outline_width"));
                dst.set_message("outline_width", dst_outline_wodth);

                dst.set_enum("outline_width_unit", src.get_enum("radius_unit"));
            }
        }
        return true;
    };
...
namespace migration_protocol_oneof
{

bool migrate_raster_provider(const DynamicMessage& src, DynamicMessage* dst)
{
    auto provider_type = src.get_enum("type");

    if (provider_type == "UNTILED_RASTER_PROVIDER")
    {
        dst->copy_message("untiled", src.get_message("untiled"));
    }
    else if (provider_type == "TILED_RASTER_PROVIDER")
    {
        dst->copy_message("tiled", src.get_message("tiled"));
    }
    else if (provider_type == "BING_RASTER_PROVIDER")
    {
        dst->copy_message("bing", src.get_message("bing"));
    }
    else if (provider_type == "PALETTIZED_RASTER_PROVIDER")
    {
        dst->copy_message("palettized", src.get_message("palettized"));
        auto src_inner_provider = src.get_message("palettized").get_message("provider");
        auto dst_inner_provider = dst->get_message("palettized").get_message("provider");
        return migrate_raster_provider(src_inner_provider, &dst_inner_provider);
    }
    else if (provider_type == "ARCGIS_RASTER_PROVIDER")
    {
        dst->copy_message("arcgis", src.get_message("arcgis"));
    }
    else if (provider_type == "TMS_RASTER_PROVIDER")
    {
        dst->copy_message("tms", src.get_message("tms"));
    }
    else if (provider_type == "WMTS_RASTER_PROVIDER")
    {
        dst->copy_message("wmts", src.get_message("wmts"));
    }
    else if (provider_type == "WMS_RASTER_PROVIDER")
    {
        dst->copy_message("wms", src.get_message("wms"));
    }
    else if (provider_type == "CESIUM_TERRAIN_RASTER_PROVIDER")
    {
        dst->copy_message("cesium_terrain", src.get_message("cesium_terrain"));
    }
    else if (provider_type == "TILEJSON_RASTER_PROVIDER")
    {
        dst->copy_message("tilejson", src.get_message("tilejson"));
    }
    else if (provider_type == "PMTILES_RASTER_PROVIDER")
    {
        dst->copy_message("pmtiles", src.get_message("pmtiles"));
    }

    return true;
}

bool migrate_nodata_value(const DynamicMessage& src, DynamicMessage* dst)
{
    auto nodata_type = src.get_enum("type");

    if (nodata_type == "BIT_PATTERN_NODATA")
    {
        dst->set_uint32("bit_pattern", src.get_uint32("bit_pattern"));
    }
    else if (nodata_type == "COLOR_NODATA")
    {
        dst->copy_message("color", src.get_message("color"));
    }
    else if (nodata_type == "INT_VALUE_NODATA")
    {
        dst->set_int32("int_value", src.get_int32("int_value"));
    }
    else if (nodata_type == "UINT_VALUE_NODATA")
    {
        dst->set_uint32("uint_value", src.get_uint32("uint_value"));
    }
    else if (nodata_type == "FLOAT_VALUE_NODATA")
    {
        dst->set_float("float_value", src.get_float("float_value"));
    }
    else if (nodata_type == "NAN_NODATA")
    {
        dst->set_message("float_nan", dst->create_message("HrzProtocol.Void"));
    }

    return true;
}

bool migrate_layer_visibility_constraint(const DynamicMessage& src, DynamicMessage* dst)
{
    auto constraint_type = src.get_enum("type");

    if (constraint_type == "BOUNDS")
    {
        dst->copy_message("bounds", src.get_message("bounds"));
    }
    else if (constraint_type == "ALTITUDE")
    {
        dst->copy_message("altitude", src.get_message("altitude"));
    }

    return true;
}

bool migrate_palette(const DynamicMessage& src, DynamicMessage* dst)
{
    auto palette_type = src.get_enum("type");

    if (palette_type == "LABEL")
    {
        dst->copy_message("label", src.get_message("label"));
    }
    else if (palette_type == "NUMERIC")
    {
        dst->copy_message("numeric", src.get_message("numeric"));
    }

    return true;
}

bool migrate_tiling_scheme(const DynamicMessage& src, DynamicMessage* dst)
{
    auto scheme_type = src.get_enum("type");

    if (scheme_type == "GLOBAL")
    {
        dst->copy_message("global_tiling", src.get_message("global_tiling"));
    }
    else if (scheme_type == "LOCAL")
    {
        dst->copy_message("local_tiling", src.get_message("local_tiling"));
    }
    // "UNKNOWN" is left with the oneof unset.

    return true;
}

bool migrate_vector_data_source(const DynamicMessage& src, DynamicMessage* dst)
{
    auto provider_type = src.get_enum("provider_type");

    if (provider_type == "TILED_VECTOR_DATA_PROVIDER")
    {
        dst->copy_message("tiled_data_provider", src.get_message("tiled_data_provider"));
    }
    else if (provider_type == "CLIENT_VECTOR_DATA_PROVIDER")
    {
        dst->copy_message("client_data_provider", src.get_message("client_data_provider"));
    }
    else if (provider_type == "IN_MEMORY_VECTOR_DATA_PROVIDER")
    {
        dst->copy_message("in_memory_data_provider", src.get_message("in_memory_data_provider"));
    }
    else if (provider_type == "TILEJSON_VECTOR_DATA_PROVIDER")
    {
        dst->copy_message("tilejson_data_provider", src.get_message("tilejson_data_provider"));
    }
    else if (provider_type == "UNTILED_VECTOR_DATA_PROVIDER")
    {
        dst->copy_message("untiled_data_provider", src.get_message("untiled_data_provider"));
    }
    else if (provider_type == "PMTILES_VECTOR_DATA_PROVIDER")
    {
        dst->copy_message("pmtiles_data_provider", src.get_message("pmtiles_data_provider"));
    }

    return true;
}

bool migrate_vector_repr(const DynamicMessage& src, DynamicMessage* dst)
{
    auto repr_type = src.get_enum("type");

    if (repr_type == "EXTRUDED_GEOMETRY_VECTOR_REPR")
    {
        dst->copy_message("extruded_geometry", src.get_message("extruded_geometry"));
    }
    else if (repr_type == "MODEL_VECTOR_REPR")
    {
        dst->copy_message("model", src.get_message("model"));
    }
    else if (repr_type == "CYLINDER_VECTOR_REPR")
    {
        dst->copy_message("cylinder", src.get_message("cylinder"));
    }
    else if (repr_type == "FLAT_OVERLAY_POINT_VECTOR_REPR")
    {
        dst->copy_message("flat_overlay_point", src.get_message("flat_overlay_point"));
    }
    else if (repr_type == "FLAT_OVERLAY_POLYLINE_VECTOR_REPR")
    {
        dst->copy_message("flat_overlay_polyline", src.get_message("flat_overlay_polyline"));
    }
    else if (repr_type == "FLAT_OVERLAY_POLYGON_VECTOR_REPR")
    {
        dst->copy_message("flat_overlay_polygon", src.get_message("flat_overlay_polygon"));
    }
    else if (repr_type == "HEATMAP_VECTOR_REPR")
    {
        dst->copy_message("heatmap", src.get_message("heatmap"));
    }
    else if (repr_type == "SYMBOL_VECTOR_REPR")
    {
        dst->copy_message("symbol", src.get_message("symbol"));
    }
    // "NULL_VECTOR_REPR" is left with the oneof unset.

    return true;
}

bool migrate_transform_symbol_component(const DynamicMessage& src, DynamicMessage* dst)
{
    auto component_type = src.get_enum("type");

    if (component_type == "TRANSFORM_SYMBOL_TRANSLATION")
    {
        dst->copy_message("translation", src.get_message("translation"));
    }
    else if (component_type == "TRANSFORM_SYMBOL_ROTATION")
    {
        dst->copy_message("rotation", src.get_message("rotation"));
    }
    else if (component_type == "TRANSFORM_SYMBOL_SCALING")
    {
        dst->copy_message("scaling", src.get_message("scaling"));
    }
    else if (component_type == "TRANSFORM_SYMBOL_GENERIC")
    {
        dst->copy_message("generic", src.get_message("generic"));
    }

    return true;
}

bool migrate_symbol_element(const DynamicMessage& src, DynamicMessage* dst)
{
    auto element_type = src.get_enum("type");

    if (element_type == "PLACEHOLDER_SYMBOL_ELEMENT")
    {
        dst->copy_message("placeholder", src.get_message("placeholder"));
    }
    else if (element_type == "ANCHOR_SYMBOL_ELEMENT")
    {
        dst->copy_message("anchor", src.get_message("anchor"));
    }
    else if (element_type == "STACK_SYMBOL_ELEMENT")
    {
        dst->copy_message("stack", src.get_message("stack"));
    }
    else if (element_type == "IMAGE_SYMBOL_ELEMENT")
    {
        dst->copy_message("image", src.get_message("image"));
    }
    else if (element_type == "PADDING_SYMBOL_ELEMENT")
    {
        dst->copy_message("padding", src.get_message("padding"));
    }
    else if (element_type == "FLEX_SYMBOL_ELEMENT")
    {
        dst->copy_message("flex", src.get_message("flex"));
    }
    else if (element_type == "SIZED_BOX_SYMBOL_ELEMENT")
    {
        dst->copy_message("sized_box", src.get_message("sized_box"));
    }
    else if (element_type == "FLEXIBLE_SYMBOL_ELEMENT")
    {
        dst->copy_message("flexible", src.get_message("flexible"));
    }
    else if (element_type == "STACK_EXPAND_SYMBOL_ELEMENT")
    {
        dst->copy_message("stack_expand", src.get_message("stack_expand"));
    }
    else if (element_type == "ALIGN_SYMBOL_ELEMENT")
    {
        dst->copy_message("align", src.get_message("align"));
    }
    else if (element_type == "CONSTRAINED_BOX_SYMBOL_ELEMENT")
    {
        dst->copy_message("constrained_box", src.get_message("constrained_box"));
    }
    else if (element_type == "ROTATED_BOX_SYMBOL_ELEMENT")
    {
        dst->copy_message("rotated_box", src.get_message("rotated_box"));
    }
    else if (element_type == "DECORATED_SHAPE_SYMBOL_ELEMENT")
    {
        dst->copy_message("decorated_shape", src.get_message("decorated_shape"));
    }
    else if (element_type == "ASPECT_RATIO_SYMBOL_ELEMENT")
    {
        dst->copy_message("aspect_ratio", src.get_message("aspect_ratio"));
    }
    else if (element_type == "FITTED_BOX_SYMBOL_ELEMENT")
    {
        dst->copy_message("fitted_box", src.get_message("fitted_box"));
    }
    else if (element_type == "TRANSFORM_SYMBOL_ELEMENT")
    {
        dst->copy_message("transform", src.get_message("transform"));
    }
    else if (element_type == "TEXT_SYMBOL_ELEMENT")
    {
        dst->copy_message("text", src.get_message("text"));
    }
    else if (element_type == "OPTIONAL_SYMBOL_ELEMENT")
    {
        dst->copy_message("optional", src.get_message("optional"));
    }
    else if (element_type == "VARIANT_SYMBOL_ELEMENT")
    {
        dst->copy_message("variant", src.get_message("variant"));
    }
    else if (element_type == "LEADER_LINE_SYMBOL_ELEMENT")
    {
        dst->copy_message("leader_line", src.get_message("leader_line"));
    }

    return true;
}

bool migrate_in_memory_attribute_values(const DynamicMessage& src, DynamicMessage* dst)
{
    const int features_count = src.field_size("features");
    for (int f = 0; f < features_count; ++f)
    {
        const auto src_feature = src.get_repeated_message("features", f);
        auto dst_feature = dst->get_repeated_message("features", f);

        const int attribute_values_count = src_feature.field_size("attribute_values");
        dst_feature.clear_field("attribute_values");
        for (int a = 0; a < attribute_values_count; ++a)
        {
            auto dst_attribute_value = dst_feature.create_message("HrzProtocol.AttributeValue");
            const auto src_attribute_value =
                src_feature.get_repeated_message("attribute_values", a);

            if (src_attribute_value.has_field("boolean_value"))
            {
                dst_attribute_value.set_bool(
                    "boolean_value", src_attribute_value.get_bool("boolean_value"));
            }
            else if (src_attribute_value.has_field("int64_value"))
            {
                dst_attribute_value.set_sfixed64(
                    "int64_value", src_attribute_value.get_sfixed64("int64_value"));
            }
            else if (src_attribute_value.has_field("uint64_value"))
            {
                dst_attribute_value.set_fixed64(
                    "uint64_value", src_attribute_value.get_fixed64("uint64_value"));
            }
            else if (src_attribute_value.has_field("number_value"))
            {
                dst_attribute_value.set_double(
                    "number_value", src_attribute_value.get_double("number_value"));
            }
            else if (src_attribute_value.has_field("string_value"))
            {
                dst_attribute_value.set_string(
                    "string_value", src_attribute_value.get_string("string_value"));
            }

            dst_feature.add_repeated_message("attribute_values", dst_attribute_value);
        }

        assert(dst_feature.field_size("attribute_values") == attribute_values_count);
    }
...
bool migration_37dc6665_to_c8253def(const DynamicMessage& src, DynamicMessage* dst)
{
    using namespace migration_protocol_oneof;

    // dtm_raster / imagery_raster: RasterProvider, NodataValue (nested in the raster's
    // nodata), TilingSchemeParams (nested in the raster provider's geometry), and
    // LayerVisibilityConstraint.
    auto migrate_raster_layer_fn = [&](const DynamicMessage& src, DynamicMessage* dst) -> bool
    {
        ...
        if (!migrate_raster_provider(src_provider, &dst_provider))
        {
            return false;
        }

        return walk_fields_of_type("HrzProtocol.NodataValue", src, dst, migrate_nodata_value)
            && walk_fields_of_type(
                   "HrzProtocol.TilingSchemeParams", src, dst, migrate_tiling_scheme)
            && walk_fields_of_type(
                   "HrzProtocol.LayerVisibilityConstraint", src, dst,
                   migrate_layer_visibility_constraint);
    };

    if (!visit_layers_of_type("dtm_raster", src, dst, migrate_raster_layer_fn)
        || !visit_layers_of_type("imagery_raster", src, dst, migrate_raster_layer_fn))
    {
        return false;
    }

    if (!visit_layers_of_type(
            "single_model", src, dst,
            [&](const DynamicMessage& src, DynamicMessage* dst) -> bool
            {
                return walk_fields_of_type(
                    "HrzProtocol.LayerVisibilityConstraint", src, dst,
    ...
    if (!visit_layers_of_type(
            ...
            {
                return walk_fields_of_type(
                    "HrzProtocol.VectorDataSource", src, dst, migrate_vector_data_source);
            }))
    {
        return false;
    }
    ...
    if (!visit_layers_of_type(
            "in_memory_vector_source", src, dst, migrate_in_memory_attribute_values))
    {
        return false;
    }
