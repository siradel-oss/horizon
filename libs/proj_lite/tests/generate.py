...
def generate_case(suite_name, case_id, in_proj, out_proj, in_x, in_y, in_z, eps):
    ...
    eps_f = float(eps)
    p = subprocess.Popen(
        'cs2cs -f "%.16f" ' + in_proj + " +to " + out_proj, stdin=PIPE, stdout=PIPE
    )

    offsets = [[0, 0], [-100 * eps_f, -100 * eps_f], [100 * eps_f, 100 * eps_f]]
    x = [0, 0, 0]
    y = [0, 0, 0]
    z = [0, 0, 0]
    ...
    for i in range(0, 3):
        p.stdin.write(
            bytes(
                "%f %f %f\n" % (in_x + offsets[i][0], in_y + offsets[i][1], in_z),
                "utf-8",
            )
        )
